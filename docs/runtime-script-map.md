...
| Script or group | Purpose | Invocation route | Key dependencies | Status |
|---|---|---|---|---|
| `subconscious_jobs.py`, `subconscious_jobs_config.py`, `subconscious_scheduler.py`, `subconscious_worker.py`, `question_tracking.py`, `journey_tracking.py`, `compensatory_dream.py` | Schedule and run background semantic/subconscious jobs, including deterministic Phase 2 question-link tracking, source-backed Journey P1-P3 helpers, and review-only Phase 1 compensatory dream candidates. | CLI, scheduler, opt-in maintenance. | Registry, model client, route metadata, job validation, source-backed question candidates, journey waypoints, extraction rows, and structurally audited dream candidate refs. | Public entrypoint |
