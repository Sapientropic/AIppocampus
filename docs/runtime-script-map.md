...
| `retrieval.py`, `retrieval_query_policy.py`, `search_clean_source.py`, `search_rollout.py` | Source-backed retrieval and raw audit fallback. | Prompt hook, MCP, CLI search surfaces. | Clean-source JSONL, registry rows, query policy. | Runtime internal |
...
| `subconscious_review.py`, `memory_candidate_router.py`, `subconscious_job_plan.py`, `subconscious_job_validation.py`, `subconscious_job_circuits.py`, `correction_reconsolidation.py`, `coding_decision_events.py` | Review, route, plan, validate, and adjudicate findings before promotion, including correction activation/outcome evidence and coding decision candidates. | Worker, maintenance CLI, and tests. | Working-memory rules, source refs, candidate schemas, privacy-scanned append-only event rows, clean-source message refs. | Runtime internal |
