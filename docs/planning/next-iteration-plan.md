...
| Cognitive runtime evidence | #163, #310, #313, #574, #575, #576, #663 | Keep open, but cut tightly. Existing substrate/report slices do not prove live/private or user-visible lift. Focus next PRs on Dream behavior evidence, Journey hint timing, thread-story / cognitive-portrait live probes, live/feedback-reviewed load calibration, private/live Observatory evidence, and Episode/Arc richer adapters / live behavior evidence. The #575 private-history aggregate runner now measures local clean-source load signals, but it still lacks reviewed false-positive/caution-usefulness and live host timing. The deterministic query-pattern route sidecar writer, default onboarding registry-metadata and reviewed-semantic-trigger route publication, prompt-hook scent consumer, alias-free Observatory readout, and #574 alias-source fixture for reviewed/generated natural multilingual hits exist, but they still do not prove live DeepSeek alias quality, scheduler adoption, or latency savings. Episode/Arc now also has a #663 private-history aggregate adjudication row for rejected-route chains; packet routes still are not source evidence, and old route currentness still requires source reopen. |
