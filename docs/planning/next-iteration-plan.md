...
|---|---|---|
| Question continuity | #248 | Close as a bounded public-safe owner track after #1367/#1368/#1369. The public shadow now covers extraction/linking/source-reopen/negative controls, no-question versus question-aware recall, stale-question carryover, missed resurfacing, wrong-route drag, noise false positives, and dynamic false merge/split checks. Future private/live/default question quality should be a new scoped issue. |
| Cognitive runtime evidence | #163, #575, #576, #663 | Close the current bounded owner tracks. #163 has public Dream closeout review for #1364/#1365 plus #1366; #575 closes diagnostic-only after #1373-#1375 and the default-path replay; #576 closes its read-only/usefulness/privacy Observatory slices #1376-#1378; #663 closes after #1361-#1363 and the public route-producer fixture. Future live/private/user-visible lift needs new narrower issues, not reopening these broad owners by default. |
| Usefulness and live route production | #1185, #1188 | Close as bounded owner tracks. #1185 has the shared usefulness contract, safe-but-useless blockers, and #1384 natural-handoff/default-session validation with wins/no-help/regressions. #1188 has gate cleanup, public/holdout cohort, explicit `agent recall --attention-router-mode auto`, multilingual/deictic fixtures, and safe hint inputs. Future broad default-hook/live-host quality remains a new scoped evidence issue. |
| Yi macro semantics hardening | #1389 | Closed after the 2026-06-14 macro recheck and cross-grain semantics slice. The #1211-#1221 landing chain and #1390-#1393 hardening children are closed; future macro work should be a new narrow product-quality or default-promotion issue, not this owner. |
| Local/global compatibility hardening | #1394 | Recommend close after GitHub issue-state review. As of the 2026-06-14 local review, the branch has the typed Section/restriction/time-window helper, obstruction-cause separation, adjudicated metric scaffolding, Shi/Ying V0 decision, and partial-glue narrowing protocol in `aippocampus_runtime.navigation.local_global_*`, with the canonical contract in `docs/architecture/local-global-compatibility.md` and regression coverage in `tests/aippocampus/test_local_global_compatibility.py`. Do not keep #1395-#1398 or #1406 open unless a fresh comment names a narrower missing behavior. |
| Parallel derivation compatibility | #1399 | Recommend close after GitHub issue-state review. As of the 2026-06-14 local review, the branch has `parallel_derivation_bundle`, source-basis alignment, compatibility/degrade arbitration, dependency-DAG diagnostics, #1316 cross-derivation fixtures, source-shape descriptor bridging, and pre-flattening consumption by three-powers, navigation-potential, and macro-router context. Canonical contract: `docs/architecture/parallel-derivation-compatibility.md`; tests: `tests/aippocampus/test_parallel_derivation_bundle.py`. Keep future source-shape spine work on #1417+ rather than reopening #1400-#1405 by default. |
| Source-backed avatar illumination | #1407 | Closed after the 2026-06-14 source-backed avatar illumination lifecycle slice. Keep future avatar work on a new narrow foreground-packet or product-quality issue, not by reopening #1407 as a broad posture owner. |
| Opt-in boundary and onboarding audit | #1435 | Recommend close after docs-health review. `docs/architecture/opt-in-continuity-boundary-audit.md` now owns the short classification table for runtime and public onboarding boundaries, links the implemented #1432/#1433 pilot where manual append becomes opt-in automatic production with auditability, and points README / Start Here / Agent Context / Install Guide toward agent-mediated plugin setup, hook rollback, one LLM-key choice, useful no-key fallback, and diagnostics/manual CLI as fallback rather than headline UX. |
| Behavior-pack and private dogfood evidence | #279 | Closed on GitHub after the remaining recall/readiness slice. Do not keep #279 in the current open queue. If private/local E2E50 needs more reviewed retained cases, ordinary post-compaction scoring, or ablation arms, open a new scoped follow-up instead of reviving this closed owner by default. |
| Benchmark and retrieval proof | #1043, #1323, #1327, #1424 | Close the current bounded owners. #1043 closes as a documented defer decision because the official locked eval endpoint/deployments are unavailable. #1323/#1327/#1424 close from the LongMemEval factual-alias 500Q replay, which records candidate coverage, miss families, fused top-k, cache/latency, zero provider calls, and zero fused regressions while preserving source-reopen boundaries. #309 is no longer the active umbrella after `source-joined-routing-decision-2026-06-14.md`; Track D #1351 and children #1352-#1360 are closed. |

The 2026-06-10 #281 public fixture validation readout retires the public
fresh-thread closeout blocker. Future live host, private real-history, or
external public-dataset fresh-thread quality work should be cut as a new scoped
issue rather than reopening #281 as a broad product-quality bucket.

Current owner queue after the 2026-06-14 closeout should not keep broad owner
issues open merely because future product-quality work exists. #1323, #1327,
#1424, #163, #248, #575, #576, #663, #1043, #1185, and #1188 move out of the
active owner queue as bounded closeouts or documented defers. Future exact-line
reranker quality, default-hook/live-host quality, or official external
benchmark access should be filed as new scoped issues with replayable evidence
requirements. #309, #1407, #1389, #1351, #1394, #1399, and #1435 are also out
of the current owner queue once their GitHub issue-state closeout is confirmed.
Closed #279 remains out.
...
15. Cognitive portrait structured-text benchmark
   ...
   - First #313 no-leakage thread-story packet diagnostic is implemented in
     ...
     source-reopened arms. The 2026-06-10 public-shadow closeout report adds
     unrelated-story noise control, issue-level closeout eligibility, and
     explicit cannot-claim boundaries for private quality, live model-family
     behavior, default recall/AAR lift, user-visible improvement, and source
     truth from the packet.
...
16. Journey Tracking P1-P3 core
   ...
   - The fixture smoke compares Journey frontier/state against a plain summary
     ...
     negative controls. It also exposes a public-safe time-sliced Journey replay
     report over a replayable public-style cohort covering active hints,
     resolved/stale/wrong-route suppression, false foreground hints, and
     no-future-leak controls; the report emits only counts and hint decisions and
     keeps raw rows, source refs, future rows, and private route handles out of
     the public payload.
   - This is still not proof of user-calibrated `theme_emergence`, production
     ...
     outputs when the job circuit exists, then evaluate live/private host timing
     and user-visible usefulness before surfacing Journey hints by default.
...
17. Dream runtime substrate plus bounded sleep-cycle path
   ...
   - The public #163 shadow report is implemented in
     `skills/aippocampus/scripts/aippocampus_runtime/dream/public_shadow_report.py`:
     it compares selected public Dream-vs-baseline cases across route lift,
     useful action deltas, verification-cost deltas, no-harm behavior, and
     wrong-hint suppression. Treat it as public reproducibility for the axes,
     not live or private-history Dream quality.
   - Next hardening: run #163's reviewed private-history or live delivered-quality
     eval with no-dream baselines, negative controls, and sanitized aggregate
     output before claiming private Dream quality, predictive value,
