...
|---|---|---|
| Question continuity | #248 | Keep open, but narrowed. Extraction gates, prefilter reporting/parity, source-ref rejoin, fallback, selected answer-quality review plumbing, default borderline auto-materialization, stale-confirmation tolerance, question-blind structural baseline reporting, theme-resonance absence reporting, and a 2026-06-10 public/source-replayable shadow fixture now exist. The public shadow retires the immediate reproducibility gap for extraction/linking/source-reopen/negative-control cases, but it is not closeout: the remaining blockers are a fair no-question-aware retrieval/answer baseline, broader private-history calibration, richer candidate/link/theme materialization, and theme/user-review lift evidence. |
| Cognitive runtime evidence | #163, #575, #576, #663 | Keep open, but cut tightly. Existing substrate/report slices do not prove live/private or user-visible lift. Focus next PRs on Dream reviewed delivered-quality evidence, live/private load calibration, private/live Observatory evidence, and Episode/Arc richer adapters / live behavior evidence. The #163 public Dream-vs-baseline shadow report now covers selected route-lift, useful-action, wrong-hint suppression, no-harm, and verification-cost axes, but it still lacks live/private reviewed delivered-quality evidence. The #313 public structured-text closeout report covers thread-story leakage, contradiction, persona, interference/noise controls, packet-only factual blocking, and source-reopened answer comparison without needing private story quality; do not treat it as live/private quality or default recall lift. The #575 private-history aggregate runner measures local clean-source load signals, and the public behavior-trace fixture covers selected reviewed usefulness/drag/risk outcomes, but it still lacks live/private reviewed host-timing quality. Episode/Arc has a #663 private-history aggregate adjudication row for rejected-route chains and a public gappy-chain fixture; packet routes still are not source evidence, and old route currentness still requires source reopen. |
| Behavior-pack and private dogfood evidence | #279 | Keep open as the private/local E2E50 follow-up owner. The public-safe behavior pack is now the primary executable path, and the wide private scanner found enough candidates, but local annotation retained only 7 control/seed cases against the 20-case private target. Next work should add reviewed private retained cases, ordinary post-compaction behavior scoring, and ablation arms only if they improve the product claim instead of chasing a case count. |
| Benchmark and retrieval proof | #309, #1043 | Keep open as falsifiable evidence owners. Do not close with architecture prose. #309 has deterministic source-joined candidate-funnel, route-chain/actionability, and public score-fusion calibration evidence for exact-quote guard, semantic bridge, wrong-stance lure suppression, vector-off fallback, and source-join rejection; it still needs measured real vector/graph/rerank consumers, latency/cost/failure examples, and a decision before any default vector/filter behavior. #1043 remains blocked on a matched no-memory STATE-Bench task run with the locked evaluation endpoint/deployments configured. |
...
Current open queue after this pass should keep #1043, #663, #576, #575, #309,
#279, #248, and #163.
