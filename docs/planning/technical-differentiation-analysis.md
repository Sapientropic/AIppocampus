...
| compensatory dream Phase 1 + P2 substrate | first deterministic prototype | `compensatory_dream.py` 已能从 source-backed 单线程 extraction rows 生成 adjudication-only `dream_synthesized` 补偿性候选，并为每条 bridge claim 保留同线程 source refs；`dream_input_pack.py` 可把 source-backed question links / Journey rows 与 ambient residue 弱提示打成跨线程 dream input pack；`dream_working_memory.py` 会在后台 structural adjudication 后才允许 dream hypothesis 进入 working memory。registry 解析、prospective / validated amplification / active imagination、真实历史 dream 质量和 recall/reflection-space 影响仍未实现。 |
