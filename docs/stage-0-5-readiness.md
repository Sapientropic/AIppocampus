...
- #66 adds a deterministic synthetic Track D runner for compaction-continuity
  ...
  the measurement and event surfaces, but they do not claim live hook capture,
  live semantic adjudication quality, or private real-history correction
  survival.
...
- #64 adds the first deterministic compensatory dream Phase 1 helper. It emits
  review-only `dream_synthesized` candidates from source-backed single-thread
  extraction rows, carries thread-scoped source refs on every bridge claim,
  discards unsourced/prior-dream rows and cross-thread refs, and keeps trigger
  defaults lower than extraction and out of foreground hooks. It does not claim
  registry-resolved clean-source refs, prospective analysis, amplification,
  active imagination, live dream quality, or recall/reflection-space influence
  before review.
