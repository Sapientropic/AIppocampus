...
| `skills/aippocampus/scripts/subconscious_jobs.py` | 680 | Subconscious job circuit runner, tool-loop wiring, provider route metadata, sample ordering, and append-only staging writes | Split route binding or write serialization only if more job classes add provider-specific policy beyond the shared route capability layer. |
