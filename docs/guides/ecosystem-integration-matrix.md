...
| OpenAI Agents SDK | Planned/unverified SDK integration. No repository adapter or official-support claim exists today. | Future work may wrap local CLI/MCP as an agent tool after checking current official SDK docs and adding a smoke. | Do not forward private registry paths or raw transcripts to hosted tools. Keep source-backed ids separate from host-private metadata. | Not characterized. | Upstream SDK/API changes, tool-contract mismatch, or lack of local operator consent degrade to no native claim. | No dedicated smoke. | Blocker: verify current official SDK surface and add a runnable adapter smoke. |
...
| Browser chat capture or companion flows | Prototype browser companion can explicitly export redacted generic JSONL. It is not an official browser-chat integration and does not auto-import. | User-enabled local capture/search in the prototype, then explicit `aippocampus import conversation --format generic-jsonl`. | Capture is off by default; data stays in browser local storage until the user exports a local file. Credentials, local paths, and injection-like text are redacted in the prototype export. | Local browser foreground interaction. No background service or remote browser API is claimed. | DOM drift, disabled capture, local storage loss, malformed export, or assistant-only turns degrade to no import or validation errors. | Covered by `tests/aippocampus/test_browser_memory_companion.py`; current browser DOM stability is not proved. | See [../../examples/browser-memory-companion/README.md](../../examples/browser-memory-companion/README.md). |
...
imports. Public copy must not say LangGraph, AutoGen, CrewAI, OpenAI Agents SDK,
Cursor, VS Code, or browser chat products have native AIppocampus support until
their rows gain host-specific examples and smokes.
