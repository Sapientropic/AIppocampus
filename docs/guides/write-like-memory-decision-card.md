...
| --- | --- | --- | --- | --- | --- |
| Mark whether a route helped or was wrong | `aippocampus agent feedback <route_id> --outcome helped --json` | receipt only by default; JSONL only with `--feedback-jsonl` | durable only with an explicit path | low-authority navigation metadata | keep working; reopen source before claims |
...
| Prepare action-time hints | `aippocampus hooks action refresh-cache --write --json` | cache only | prepared local navigation cache | navigation only | install/status the action hook if desired |
| Backfill source-trailed continuity domains | `aippocampus continuity-domain produce --preview --json` | preview first; append/publish is operator work | source-ref route snapshot when published | reopenable route, not fact text | review preview and source refs before publish |
