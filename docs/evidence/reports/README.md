...
Role: dated evidence report layer.
Status: provenance archive for promoted aggregate reports; not the current claim
snapshot.

This folder holds dated, public-safe or aggregate-only evidence reports that do
not belong to a narrower family folder such as `benchmarks/`, `dream/`,
`question/`, `readiness/`, or `security/`.

Start from [`../README.md`](../README.md),
[`../current-claims.md`](../current-claims.md), or
[`../benchmark-evidence-map.md`](../benchmark-evidence-map.md) before reading
these reports as support for present-tense claims.
