...
This dated report records separate deterministic measurements for the public longitudinal-memory benchmark surfaces added for PR #173. Raw command outputs were written under `.tmp/public-longitudinal-measurements/`; this committed report keeps only sanitized metrics and claim boundaries.
...
- LoCoMo supports public same-conversation source-evidence retrieval, not cross-conversation longitudinal-user memory or coding tacit-constraint quality.
...
- The flagship next slice should use real public VCS/rollout histories and track temporal override, implicit constraint drift, cross-project contamination, intentional forget compliance, post-compaction detail recovery, and dream semantic quality separately.
