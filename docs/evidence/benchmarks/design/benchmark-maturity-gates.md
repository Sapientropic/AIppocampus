...
| Map-rot lifecycle-debt | `public_cohort` plus retained `contract_smoke` | 270 public-safe lifecycle cases, 9 families, 68 holdout; plus 9 selected lifecycle-state smoke cases | Operator review before cleanup writes | Public/holdout gate now passes for lifecycle routing and no-write maintenance planning; it still does not claim completed cleanup or private-history map quality. |
| Agent continuity loop | `public_cohort` plus retained `contract_smoke` | 180 public-safe integration cases, 6 families, 45 holdout; plus 8 selected integration smoke cases | Runtime/private-history adoption remains separate | Public/holdout gate now passes for source-safe route/deepen/facade composition; it still does not claim live host or private-history usefulness. |
...
[`benchmark-family-promotion-candidates-2026-06-20.md`](../reports/benchmark-family/benchmark-family-promotion-candidates-2026-06-20.md)
and generated by
