...
| Host/ecosystem | `benchmark_provider_conformance.py` | synthetic provider/session, cross-provider route, and MCP affordance contract | implemented | P1 | benchmark-smoke / full benchmark | contract_smoke | Provider identity conflation, copied-summary evidence promotion, injected-content pollution, MCP blob-only recall | Live multi-client quality, all-client drop-in support, or full #981 kit | #988, #981 |
