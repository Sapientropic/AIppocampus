...
| External memory benchmark | `benchmark_longmemeval_v2_official_pilot.py` / `longmemeval_v2_aippocampus_adapter.py` | LongMemEval-V2 official-harness pilot decision and text-only Memory adapter contract | scaffold | P1 | deterministic decision report in `benchmark-smoke`; real official run optional local only | diagnostic_proxy for pilot readiness; no score until a dated official-harness run exists | Blending memory-context quality, answer accuracy, reader/evaluator dependency, latency, token use, cost, and report privacy into one overclaimed number | V2 answer accuracy, LAFS, leaderboard readiness, SOTA, source-evidence R@K/MRR, or broad memory superiority | #1155 |
