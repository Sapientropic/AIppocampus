...
current metric values in [`../../current-claims.md`](../../current-claims.md),
...
| Continuous-agent continuity | `benchmark_continuous_memory_arms.py` | no-memory, host-native, sham, stale, oracle, and true-memory arms | implemented | P1 | full benchmark / optional local | diagnostic_proxy | Unfair fresh-context baselines, stale-memory harm, hidden cost | Public superiority without preregistered evidence | #378, #406, #407, #408, #409, #410 |
...
| External memory benchmark | `benchmark_amemgym.py` / `benchmark_amemgym_official.py` | AMemGym metadata, source-backed overlay smoke, official bridge, and full `local-scripted` official-output protocol run | protocol_evidence | P1 | benchmark-smoke; optional public JSON download; optional local official bridge | diagnostic_proxy | Collapsing native accuracy, diagnosis, utilization, source fidelity, and protocol-only output values into one score | Live-model official AMemGym score or baseline parity | #733 / #742 |
...
| Public longitudinal | `benchmark_vcs_future_event_recall.py` | VCS future-event recall and source disambiguation | implemented | P1 | optional local / dated report owner | diagnostic_proxy | Future-event false negatives, closed-book contamination, source-vs-stale confusion | Live model quality or wild VCS quality | #378, #454 |
...
| Cognitive Observatory | `smoke_route_readiness_observatory.py` | implemented | P1 | focused smoke | contract_smoke | Route-readiness/prewarm diagnostics leaking into control or source-evidence territory | Complete observatory UI or live prewarm quality | #574, #576 |
...
| Warm ambient | `aippocampus_runtime.warm_ambient.query_pattern_enrichment --fixture --json` | implemented | P1 | focused smoke | contract_smoke | Registry/import query-pattern planning, idempotency, invalidation, and provider/privacy gate drift | Live DeepSeek quality, foreground-hook consumption, sidecar writes, or latency savings | #574 |
...
| Semantic source review | `smoke_semantic_scope_source_review.py` | implemented | P1 | private/sanitized or live provider smoke | selected_private_diagnostic | Source-review availability and label drift | Human-reviewed global semantic quality | none |
