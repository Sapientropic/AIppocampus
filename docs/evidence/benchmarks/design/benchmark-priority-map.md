...
| Attention router | `benchmark_attention_navigation_quality.py --profile public-cohort` | explicit-pull attention-router navigation over the 270-case public/holdout cohort, with the 12-case `--profile contract-smoke` path retained for fast red-line regression | implemented | P1 | public-cohort for evidence/readiness; contract-smoke for fast regression | narrow public_cohort completed for explicit agent-pull attention routing; default/live adoption still separate | Drift that reclassifies the promoted explicit-pull surface as unresolved because the contract-smoke profile still exists | Answer generation, private-history router quality, live host lift, broad production routing, or default foreground adoption | #1111, #1165, #1195, #1349, #1350, #1970 |
| Agent continuity loop | `benchmark_agent_continuity_loop.py` | Integrated public-safe route loop across semantic warming, hot router, facade packets, AIppo, source-reopen budget, foreground budget, deepen/explain, blocked/stale/conflict, and anti-nag cases | implemented | P1 | benchmark-smoke / full benchmark | contract_smoke; #1195 public-cohort-candidate target recorded; `quality_gate_ok=false` until measured cohort, usefulness blockers, attention cost, holdout, and no-tuning-leak promotion pass | Integration drift where individually green contracts compose into overclaim, provenance leak, stale-as-current, or forgotten deepen | Live host behavior lift, private-history quality, answer generation, default foreground adoption, or public benchmark quality lift | #1163, #1165, #1195 |
...
| Benchmark infrastructure | `benchmark_family_promotion_candidates.py` | #1195 family-promotion decision report for the first public cohort candidate targets and usefulness blockers | implemented | P1 | direct runner / benchmark-smoke owner | public_cohort_candidate decision report; no observed quality score | Promoting fixture families without sample floors, holdout, no-tuning-leak, sanitization, uncertainty, or usefulness blockers | Representative public quality, holdout quality, or live/private behavior lift by itself | #1195, #1185, #279 |
...
| Field/demo continuity | `benchmark_field_continuity.py` | public-safe magic-moment scenario contracts plus supporting bounded #281 fixture-quality proxy | implemented | P0/P1 | benchmark-smoke / full benchmark | contract_smoke | Demo overclaim, wrong-family route persistence | Live/private field quality proof or #281 live fresh-thread quality | #454, #281 |
...
| Fresh-thread recall | `benchmark_fresh_thread_recall_demo.py` | fresh-thread public-safe recall/demo flows plus #281 public fixture validation readout | implemented | P1 | full benchmark; dated report owner | public_safe_claim for fixture flow | Source-reopen boundary regressions in demos, negative-control memory drag, over-personalization, and manual-query invention | Default hook first-turn live lift, private-history quality, or universal fresh-thread recall | #490, #281 |
...
| Warm ambient | `builders/build_warm_ambient_trace_cases.py` | warm ambient case-pack builder | implemented | P3 | optional local | contract_smoke | Case-pack generation drift | Quality evidence alone | none |
...
| E2E50 | `benchmark_e2e50_silent_constraint.py` | public-safe behavior pack for silent constraints, no-remember negatives, ordered sequence, load sidecar contracts, and `--field-validation` private/local retained-case gate | contract_smoke | P0/P1 | benchmark-smoke support guard | contract_smoke; `quality_gate_ok=false`; `field_validation_gate_ok=false` until #1981 retained private/local coverage reaches target | Scorer/report drift, private case-count overblocking, order-insensitive route evidence, summary-overhang traps, load overclaim, and private/local field-lift overclaim | Representative #279 quality, #663 Episode/Arc layer, #575 load-routing quality, private-history lift, or 50-case result | #279/#663/#575/#1154/#1981 |
...
| External memory benchmark | `benchmark_longmemeval_answer.py` | LongMemEval-S fixed-reader answer, latency, token, cache, and cost telemetry over the existing V1 source-evidence pipeline | scaffold | P1 | deterministic dry-run in `benchmark-smoke`; provider path optional local only with explicit shared provider budget/checkpoint | diagnostic_proxy for answer-harness contract; no promoted answer score until a dated provider run exists | Blending retrieval, reader quality, latency, token use, cost, and report privacy into one overclaimed number | Official LongMemEval leaderboard score, official judge score, V2 quality, LoCoMo/PersonaMem quality, SOTA, default reader/provider adoption, or private-history quality | #1157/#1229 |
| External memory benchmark | `benchmark_longmemeval_v2_context.py` | LongMemEval-V2 context-mapping pilot | diagnostic_only | P1 | optional local with dataset files | diagnostic_proxy | Claiming V2 source-evidence score without evidence labels | R@K/MRR, LAFS, answer accuracy, official harness readiness | #259 |
| External memory benchmark | `benchmark_longmemeval_v2_official_pilot.py` / `longmemeval_v2_aippocampus_adapter.py` | LongMemEval-V2 official-harness pilot decision and text-only Memory adapter contract | scaffold | P1 | deterministic decision report in `benchmark-smoke`; real official run optional local only and must add provider budget before execution | diagnostic_proxy for pilot readiness; no score until a dated official-harness run exists | Blending memory-context quality, answer accuracy, reader/evaluator dependency, latency, token use, cost, and report privacy into one overclaimed number | V2 answer accuracy, LAFS, leaderboard readiness, SOTA, source-evidence R@K/MRR, or broad memory superiority | #1155/#1229 |
| External memory benchmark | `benchmark_amemgym.py` / `benchmark_amemgym_official.py` | AMemGym metadata, source-backed overlay smoke, official bridge, full `local-scripted` official-output protocol run, provider-budget guard, route preflight, and dated live-provider blocker notes | protocol_evidence_blocked_live | P1 | benchmark-smoke; optional public JSON download; optional local official bridge; `openrouter --run` requires shared provider budget/checkpoint and probes required routes before subprocess execution | diagnostic_proxy | Collapsing native accuracy, diagnosis, utilization, source fidelity, and protocol-only output values into one score | Live-model official AMemGym score or baseline parity until a declared provider/model condition completes `overall`, `upperbound`, and `random`; #1232 records the current OpenRouter OpenAI-family route blocker after top-up and harmless-prompt diagnosis | #733/#742/#1229/#1232 |
...
| External memory benchmark | `benchmark_locomo_qa.py` | LoCoMo fixed-reader text-QA answer, latency, token/cache, cost, and citation telemetry over the standard source-backed retrieval adapter | scaffold | P1 | deterministic dry-run in `benchmark-smoke`; provider path optional local only with explicit shared provider budget/checkpoint | diagnostic_proxy for answer-harness contract; no promoted answer score until a dated fixed-reader provider run exists | Blending retrieval, answer quality, citation, latency, token use, cost, and report privacy into one overclaimed number | Official LoCoMo leaderboard or judge score, multimodal LoCoMo quality, LongMemEval/PersonaMem quality, SOTA, default reader/provider adoption, or private-history quality | #1158/#1229 |
...
| Public longitudinal | `benchmark_vcs_future_event_recall.py` | VCS future-event recall, source disambiguation, and route-chain actionability | dated_evidence | P1 | optional local / dated report owner | diagnostic_proxy | Future-event false negatives, closed-book contamination, source-vs-stale confusion, incomplete multi-source chains, candidate-budget bloat, and foreground route-drag from successful current events | Live model quality, wild VCS quality, or #1195 benchmark-family promotion from the synthetic rollout cohort alone | #309, #378, #454, #961, #1197, #1195 |
...
| Recall navigation promotion | `smoke_recall_navigation_promotion.py` | implemented | P1 | focused smoke | promotion_contract | Single-variable recall-navigation A/B drift, feature hurt/no-op hiding, stale/conflict/noise/wrong-source fixture gaps, and default-promotion overclaim | Live default behavior lift, answer quality, private-history quality, or macro/router default readiness | #2559 active; #1302/#1185/#1300/#1301 historical |
