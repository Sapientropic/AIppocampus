...
| External memory benchmark | `benchmark_amemgym.py` / `benchmark_amemgym_official.py` | AMemGym metadata, source-backed overlay smoke, official bridge, full `local-scripted` official-output protocol run, and dated live-provider blocker note | protocol_evidence_blocked_live | P1 | benchmark-smoke; optional public JSON download; optional local official bridge; `openrouter --run` requires shared provider budget/checkpoint | diagnostic_proxy | Collapsing native accuracy, diagnosis, utilization, source fidelity, and protocol-only output values into one score | Live-model official AMemGym score or baseline parity until bounded live-provider execution and cost extraction exist | #733/#742/#1229 |
