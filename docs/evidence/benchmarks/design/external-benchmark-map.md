...
| AMemGym | Interactive memory benchmark with structured user profiles, state evolution, Native/RAG/AWI/AWE arms, and write/read/utilization failure analysis. | Public `v1.base` intake, deterministic metadata smoke, checked-in fixture, and source-backed overlay metrics are documented in [`../amemgym.md`](../amemgym.md). | Metadata and local overlay smoke implemented; official interactive runner and baseline arms still blocked. | Official AMemGym score, official-runner compatibility, Native/RAG/AWI/AWE parity, or treating overlay utilization flags as official diagnosis output. |
...
- AMemGym: use the public `v1.base` metadata smoke and checked-in overlay
  fixture before wiring the official interactive runner. Keep AMemGym native
  accuracy, diagnosis, utilization, source-backed overlay fidelity, and
  cost/latency separate.
