...
| MemoryAgentBench | Incremental multi-turn memory-agent benchmark covering retrieval, test-time learning, long-range understanding, and conflict resolution. | Feasibility intake is documented in [`../memoryagentbench.md`](../memoryagentbench.md). The accepted path is metadata/case-pack smoke first, then incremental replay only after write/update/forgetting contracts are explicit. | Feasibility accepted; implementation deferred to #608. | Any AIppocampus score, official-runner compatibility, fairness claim, or static-retrieval substitute for test-time learning/conflict-resolution quality. |
...
- MemoryAgentBench: use #608 as the narrow Stage 1/2 follow-up for a deterministic
  metadata and public-safe case-pack smoke. Keep full incremental replay out of
  scope until ingestion/update/forgetting contracts are measurable without
