# Dream Evidence

Audience: Dream feature maintainer / evidence reviewer.
Read this when: checking dated Dream/subconscious candidate evidence or whether a Dream-related claim has public-safe support.
Skip to: `../current-claims.md` for current claim status; `../../research/dream-task-design.md` for design intent; `../../architecture/runtime/cognitive-runtime-architecture.md` for runtime boundaries.

This folder keeps dated Dream evidence reports and companion JSON artifacts.
Use [`../current-claims.md`](../current-claims.md) for the current claim
snapshot before treating any dated report as current.

Current and recent report entries:

- [`dream-delivery-quality-eval-2026-06-14.md`](dream-delivery-quality-eval-2026-06-14.md):
  public-safe three-arm Dream delivery-quality eval for #1438.
- [`dream-atlas-live-pilot-2026-06-12.md`](dream-atlas-live-pilot-2026-06-12.md):
  public-safe live DeepSeek V4 Flash atlas pilot for #1286.
- [`dream-private-large-history-diagnostic-2026-06-04.md`](dream-private-large-history-diagnostic-2026-06-04.md):
  aggregate private-history diagnostic with public-safe summary only.
- [`dream-real-history-model-backed-eval-2026-05-31.md`](dream-real-history-model-backed-eval-2026-05-31.md):
  selected real-history model-backed Dream eval.
- [`dream-live-shadow-benchmark-corpus-2026-05-31.md`](dream-live-shadow-benchmark-corpus-2026-05-31.md):
  public Dream live-shadow benchmark corpus.
- [`dream-live-shadow-ab-2026-05-30.md`](dream-live-shadow-ab-2026-05-30.md):
  early public live-shadow A/B report.

Do not add raw private history, local paths, API keys, raw source refs, or
foreground delivery claims to this folder.
