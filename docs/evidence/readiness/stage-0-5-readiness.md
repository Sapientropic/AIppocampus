...
| --- | --- | --- |
| 0 | Ubuntu PR CI runs a canonical Python 3.12 lane with docs health, Ruff `E9` + full Pyflakes `F`, mypy, compile checks, coverage, and `python tools/aippocampus/run_tests.py --tier pr` for the broad deterministic regression path. CI also runs a Python 3.13 `quick` compatibility lane and a focused macOS default TMPDIR path-identity gate for recurring #140/#242/#402 `/var` and `/private/var` regressions. `--tier quick` is the small local inner loop, `--tier pr` is the broad pre-push lane, and `--tier full` remains the explicit release/readiness suite. | Keep the canonical Ubuntu PR lane and focused compatibility/path gates green after each slice; run the slow, benchmark, manual macOS install smoke, or full tier when claiming the surface they own |
