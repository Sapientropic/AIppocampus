...
| --- | --- | --- |
| 0 | Ubuntu PR CI runs Python 3.12/3.13 docs health, Ruff `E9` + full Pyflakes `F`, mypy, compile checks, and `python tools/aippocampus/run_tests.py --tier pr` for the broad deterministic regression path. PR CI also runs a macOS `pr`-tier gate on the runner's default TMPDIR as a path-identity guard for recurring #140/#242/#402 `/var` and `/private/var` regressions. `--tier quick` is the small local inner loop, and `--tier full` remains the explicit release/readiness suite. | Keep the Ubuntu and macOS PR gates green after each slice; run the slow, benchmark, manual macOS install smoke, or full tier when claiming the surface they own |
