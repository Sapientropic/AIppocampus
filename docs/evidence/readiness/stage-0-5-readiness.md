...
- #64 adds the first deterministic compensatory dream Phase 1 helper. It emits
  ...
  The follow-up P2 substrate builds cross-thread dream input packs from
  source-backed question links and Journey rows while treating ambient residue
  as weak source-ref fingerprints only. Background-adjudicated dream hypotheses
  can now project onto the existing working-memory substrate for
  recall/ambient/reflection consumers only when every bridge claim carries
  source refs. A selected real-history structural eval now runs over the current
  registry, selects cross-thread packs from materialized question/frontier and
  working-memory rows, emits deterministic compensatory/amplification
  candidates, adjudicates them, and compares dream-augmented substrate against
  plain rows. The 2026-05-30 local smoke observed 697 subconscious job rows,
...
| --- | --- | --- |
| 0 | Ubuntu PR CI runs a canonical Python 3.12 lane with docs health, Ruff `E9` + full Pyflakes `F`, mypy, compile checks, coverage, `python tools/aippocampus/run_tests.py --tier pr` for the fast local PR gate, and sharded `python tools/aippocampus/run_tests.py --tier broad-pr` for broad deterministic pre-merge coverage. CI also runs a Python 3.13 `quick` compatibility lane and a focused macOS default TMPDIR path-identity gate for recurring #140/#242/#402 `/var` and `/private/var` regressions. `--tier quick` is the small local inner loop, `--tier pr` is the fast pre-push lane, `--tier broad-pr` is the broad deterministic pre-merge lane, and `--tier full` remains the explicit release/readiness suite. | Keep the canonical Ubuntu PR lane and focused compatibility/path gates green after each slice; run the slow, benchmark, manual macOS install smoke, broad-pr, or full tier locally only when claiming the surface they own |
