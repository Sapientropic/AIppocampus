...
| Living cue cache smoke | `tools/aippocampus/smoke/smoke_living_cue_cache.py` and `tests/aippocampus/test_living_cue_cache.py` | Public-safe #281 fixture for learned-phrase-to-source-handle bridging, stale/temporary suppression, over-personalization diagnostics, and no-live-LLM selector output; not default-hook integration or fresh-thread quality proof. |
