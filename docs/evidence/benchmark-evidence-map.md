...
| Query-pattern enrichment no-write fixture | `aippocampus_runtime.warm_ambient.query_pattern_enrichment --fixture --json` and `tests/aippocampus/test_query_pattern_enrichment.py` | Public-safe #574 registry/import planning contract for changed-generation work items, cache reuse, idempotent existing work, digest invalidation, and provider/privacy suppression; not live DeepSeek quality, foreground-hook consumption, sidecar writes, or latency savings. |
...
| Query-pattern enrichment no-write fixture | `python -m aippocampus_runtime.warm_ambient.query_pattern_enrichment --fixture --json` | `skills/aippocampus/references/subconscious-jobs.md`, `docs/guides/public-api.md`, #574 |
