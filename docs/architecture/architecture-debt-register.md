...
| `skills/aippocampus/scripts/aippocampus_runtime/update/cli.py` | 1000 | 1000 | P1 host-readiness-risk | Split `agent_callable` / foreground-tool visibility status into a sibling update-surface helper before adding another host/package/plugin readiness lane. | #1470/#1472 kept current-thread visibility and nested plugin-cache action in status output, while plugin-cache/action text helpers moved into `update/status_actions.py`. The next host surface should not extend this facade directly. |
