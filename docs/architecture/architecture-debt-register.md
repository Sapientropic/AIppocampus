...
| `skills/aippocampus/scripts/aippocampus_runtime/mcp/server.py` | 925 | 940 | P1 agent-host-risk | Split tool-family handlers before adding another diagnostic surface, host-specific MCP projection, or write-like setup lane. | #1656/#1663/#1664/#1665/#1676 kept compact `agent_recall`/`recall_context`, request-index `agent_deepen` follow-through, and explicit `register_thread` write consent in the MCP facade because they close one existing foreground contract. #1813/#1817 kept compact tool-list and recoverable last-recall errors in the same MCP owner. #1889 kept missing-selector recovery cards in `agent_deepen`/`recall_deepen` because they are runtime validation for existing tool calls, not a new MCP family. The next MCP expansion should extract handler families rather than raising this guard again. |
| `skills/aippocampus/scripts/aippocampus_runtime/update/cli.py` | 1254 | 1280 | P1 host-readiness-risk | Split another host/plugin readiness lane before adding more package, plugin-cache, or foreground-tool status behavior. | #1470/#1472 kept current-thread visibility and nested plugin-cache action in status output, while plugin-cache/action text helpers moved into `update/status_actions.py`; #1431/#1507 moved compact agent-facing status into `update/agent_status_summary.py`. #1563 kept same-version installed-cache selection and one-command reinstall fallback in the apply facade but moved plugin-cache status projection into `update/plugin_status.py`; the 2026-06-16 frontdoor sweep kept status/plan readiness cards, Windows hidden-launch diagnostics, and #1881 no-write recovery cards here because they explain the same local update transaction before mutation. The next host surface should extract another readiness lane before extending this facade. |
| `skills/aippocampus/scripts/aippocampus_runtime/update/plugin_installer.py` | 886 | 900 | P1 host-install-risk | Split the Codex app-server JSON client or host-probe wording helper before adding another host manager, marketplace backend, or install target. | #1345 extracted Codex CLI compatibility, launcher resolution, installed-cache pathing, and cache removal into `update/codex_plugin_cli.py`; #1476 moved public-safe install projection into `update/plugin_public_summary.py`; #1785 moved public-safe uninstall projection there too. #1810 kept task-first install/status help in this installer because it owns local plugin-file mutation. #1748 kept key agent-tool smoke calls in this host probe owner because stale schemas and live tool-call failures must be reported from the same installation verification surface. The next expansion should extract the host-probe client rather than extending the installer. |
...
| `skills/aippocampus/scripts/aippocampus_runtime/ops/successor_evidence.py` | 1644 | 1700 | P0 successor-closeout-risk | Split issue manifest data, track-specific metric adapters, or GitHub native-parent loading before adding another successor wave or closeout policy. | #1918-#1981 kept live issue-scope reconciliation, nested child coverage, specific acceptance metrics, and closeout boundaries in one sweep owner so issue closure is evidence-bound instead of range-bound. The next successor wave should extract manifest/metric adapters first; do not add another broad track catalog here. |
...
| `tests/aippocampus/test_aippocampus_mcp_server.py` | 1860 | 1880 | #1450 / #1451 / #1656-#1676 / #1813 / #1817 / #1712 / #1889 / #1890 / #1982 | Split JSON-RPC request builders or repeated MCP payload assertion helpers before adding another tool family; keep host-facing compact/full/error contracts visible together while the MCP surface stabilizes. The 2026-06-16 sweep kept required-any schema, recoverable last-recall, list-threads, memory-health recovery, compact payload-budget assertions, missing-selector/runtime health foreground cards, and #1982 stale foreground MCP runtime recovery together because they guard one MCP host contract. `latest_reply` MCP closeout semantics now live in `tests/aippocampus/test_mcp_latest_reply.py`. |
| `tests/aippocampus/test_update_sync.py` | 1934 | 1950 | #1307 / #1335 / #1431 / #1563 / #1748 / #1755 / #1810 / #1982 | Split Codex config/cache fixture builders or plugin apply/status assertion helpers before adding another host install/sync lane; keep one local-update contract suite until the marketplace/cache/current-thread visibility behavior stabilizes. The Windows hidden lifecycle launcher, plugin-cache smoke assertions, key-tool live host smoke, host-conformance label tests, and #1982 current-foreground key-tool state split stay here because they exercise the same local install/update contract; split host fixture builders before adding another platform lane. |
| `tests/aippocampus/test_agent_opt_in_continuity.py` | 1776 | 1800 | #1507 / #1620 / #1808 / #1814 | Split shared CLI/MCP fixture builders or returned-packet shape assertion helpers before adding another foreground recall surface; keep opt-in recall, AIppo, macro, compact/full recall boundary, and action-card JSON contracts visible together while the agent-facing packet shape is stabilizing. Feedback/macro CLI help and receipt smoke coverage now lives in `tests/aippocampus/test_agent_feedback_macro_cli.py`; compact foreground packet assertions remain here because they guard the same agent-facing JSON shape. |
| `tests/aippocampus/test_aippocampus_cli.py` | 1778 | 1850 | #1756 / #1843 / #1838 / #1845 / #1774-#1823 | Split subprocess runner helpers or command-family smoke tests into focused modules before adding more top-level facade, personal-control, learning, or package-facade cases; keep the first-five-minutes path visible in one smoke owner. This sweep moved command-family recovery/help contracts into `tests/aippocampus/test_cli_recovery_cards.py`; the remaining growth is continuity-domain preview/read-path, learning replay/status, and self-note read smoke that still guard first-use foreground flows. Split subprocess runner helpers or another command-family module before adding a new top-level facade family. |
| `tests/aippocampus/test_aippocampus_health.py` | 1651 | 1680 | #248 / #1809 / #1782 / #1890 | Split storage-pressure fixture builders, host-state fixture builders, or product-readiness projection assertions before adding another health lane; keep generated-cache pressure advisory, host-state confound separation, and live-delta readiness together until that contract stabilizes. |
| `tests/aippocampus/test_docs_health.py` | 1587 | 1610 | #672 / #1837 / #1845 / #1846 / #1887 | Split public-doc command lint, executable memory-card examples, evidence-ledger guards, diagnostic-prerequisite guards, or product-profile guards into focused helpers before adding another docs-health domain; keep one repository health smoke owner for cross-doc drift. |
...
| `tests/aippocampus/test_warm_ambient_recall.py` | 2219 | 2250 | #657 / #1281 | Keep warm ambient hook, registry reconciliation, cache, scout, detached-job, and optional queue-status tests together while the missing-registration repair path is being stabilized. Split hook-seen ledger fixtures, queue-status fixtures, or scout-card assertion helpers before adding another scheduler/lifecycle lane. |
| `tools/aippocampus/docs/check_docs_health.py` | 1422 | 1440 | #672 / #1887 | Product profile guards now live in `tools/aippocampus/docs/product_profile_guard.py`; evidence-ledger payload line checks and #1887 diagnostic-prerequisite wording guards stayed here because they are repository docs-health invariants. Split another focused check group or shared markdown/path scanner before adding more public-readiness domains; keep single CLI output stable. |
