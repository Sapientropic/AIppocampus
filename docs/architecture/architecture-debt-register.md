...
| `tests/aippocampus/test_aippocampus_mcp_server.py` | 1655 | 1700 | #1450 / #1451 / #1656-#1676 | Split JSON-RPC request builders or repeated MCP payload assertion helpers before adding another tool family; keep host-facing compact/full/error contracts visible together while the MCP surface stabilizes. `latest_reply` MCP closeout semantics now live in `tests/aippocampus/test_mcp_latest_reply.py`. |
...
| `tests/aippocampus/test_agent_opt_in_continuity.py` | 1722 | 1740 | #1507 / #1620 / #1808 / #1814 | Split shared CLI/MCP fixture builders or returned-packet shape assertion helpers before adding another foreground recall surface; keep opt-in recall, AIppo, macro, compact/full recall boundary, and action-card JSON contracts visible together while the agent-facing packet shape is stabilizing. Feedback/macro CLI help and receipt smoke coverage now lives in `tests/aippocampus/test_agent_feedback_macro_cli.py`; compact foreground packet assertions remain here because they guard the same agent-facing JSON shape. |
| `tests/aippocampus/test_aippocampus_cli.py` | 1524 | 1560 | #1756 / #1843 / #1838 / #1845 | Split subprocess runner helpers or command-family smoke tests into focused modules before adding more top-level facade, personal-control, learning, or package-facade cases; keep the first-five-minutes path visible in one smoke owner. This sweep moved command-family recovery/help contracts into `tests/aippocampus/test_cli_recovery_cards.py` instead of raising this budget. |
| `tests/aippocampus/test_docs_health.py` | 1507 | 1560 | #672 / #1837 / #1845 / #1846 | Split public-doc command lint, executable memory-card examples, or product-profile guards into focused helpers before adding another docs-health domain; keep one repository health smoke owner for cross-doc drift. |
