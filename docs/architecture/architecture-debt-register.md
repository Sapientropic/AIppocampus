...
| `skills/aippocampus/scripts/aippocampus_runtime/mcp/recall_navigation.py` | 941 | 980 | P1 agent-recall-risk | Split topic-label selection, source-window projection, or malformed-handle diagnostics before adding another route family, handle family, or freshness policy. | #1278 and #1188 deliberately kept display-vs-callable deepen diagnostics and fixed-vocabulary route-topic labels in the progressive MCP recall owner because they directly shape the same `recall_context -> recall_deepen` packet contract. Do not add more route-label heuristics here without extracting a topic helper. |
| `skills/aippocampus/scripts/aippocampus_runtime/update/cli.py` | 973 | 1000 | P1 host-readiness-risk | Split `agent_callable` / foreground-tool visibility status into a sibling update-surface helper before adding another host/package/plugin readiness lane. | #1279 kept package artifact, MCP/plugin, CLI, and foreground agent-callable diagnostics together so the status output can explain false readiness in one place. The next host surface should not extend this facade directly. |
| `skills/aippocampus/scripts/aippocampus_runtime/update/plugin_installer.py` | 735 | 760 | P1 host-install-risk | Split the Codex app-server JSON client or CLI/result projection before adding another host manager, marketplace backend, or install target. | #1335 keeps the one-command Codex plugin install/verify/rollback orchestration in one owner because build, local marketplace, plugin-manager reinstall, MCP reload, and `sync_status` probe must stay transactionally visible. The next expansion should extract the host-probe client or marketplace writer rather than extending the installer. |
...
| `skills/aippocampus/scripts/aippocampus_runtime/navigation/local_global_compatibility.py` | 637 | 680 | P2 diagnostic-surface-risk | Split public fixture catalog construction or report/CLI projection before adding another packet family, overlap basis, or foreground consumer. | #1285 keeps normalization, deterministic compatibility evaluation, and public-safe cross-surface fixtures together because the V0 contract is one explain/deepen/Campus diagnostic surface. Do not turn it into a router, truth layer, or default foreground drawer; future live-recall consumption should extract the fixture/report edges first. |
