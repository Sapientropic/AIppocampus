...
| `skills/aippocampus/scripts/aippocampus_runtime/hooks/action_hint_cache.py` | 473 | 540 | P1 pretool-cache-risk | Split provider adapters or prepared-record matching before adding another upstream provider family. | #1609 created the prepared action-hint cache owner for #435. #1613 added learned AIppo clause records as the fifth prepared provider family because it shares the same no-model, source-ref, TTL/currentness, and suppression gates as AAR v2, learning-loop, active-lock, and attention-token records. Do not add fresh semantic/model work or a sixth provider family here without extracting a provider-specific adapter. |
...
| `tests/aippocampus/test_update_sync.py` | 1554 | 1650 | #1307 / #1335 / #1431 / #1563 | Split Codex config/cache fixture builders or plugin apply/status assertion helpers before adding another host install/sync lane; keep one local-update contract suite until the marketplace/cache/current-thread visibility behavior stabilizes. |
