...
| `skills/aippocampus/scripts/dream_real_history_eval.py` | 1250 | Selected real-history dream pack selection, deterministic fallback worker, optional bounded model-backed worker handoff, structural and user-visible ablation metrics, manual clean-source review evidence generation, sanitized aggregate output, and CLI wiring | Split manual source-review reopening and visibility-ablation scoring into focused helpers before adding live smoke orchestration, larger review slices, or additional eval surfaces. Keep model-backed prompt/validation policy in `dream_worker.py`. |
