...
| `skills/aippocampus/scripts/aippocampus_runtime/mcp/server.py` | 1068 | 1120 | P1 agent-host-risk | Split tool-family handlers before adding another diagnostic surface, host-specific MCP projection, or write-like setup lane. | #1656-#1676 kept compact `agent_recall`/`recall_context`, request-index `agent_deepen` follow-through, and explicit `register_thread` write consent in the MCP facade because they close one existing foreground contract. #2081 kept compact health exposure here because it is the existing `memory_health` tool family. The next MCP expansion should extract handler families rather than raising this guard again. |
| `skills/aippocampus/scripts/aippocampus_runtime/update/cli.py` | 1444 | 1500 | P1 host-readiness-risk | Split another host/plugin readiness lane before adding more package, plugin-cache, foreground-tool status behavior, or dirty-sync recovery. | #1431/#1507 moved compact agent-facing status into `update/agent_status_summary.py`; #1563 kept same-version installed-cache selection and one-command reinstall fallback in the apply facade but moved plugin-cache status projection into `update/plugin_status.py`; #2078 kept dirty-sync recovery in this facade because it explains the local update transaction before mutation. The next host surface should extract another readiness lane before extending this facade. |
...
| `tests/aippocampus/test_aippocampus_mcp_server.py` | 1978 | 2050 | #1450 / #1451 / #1656-#1676 / #1813 / #1817 / #1712 / #1889 / #1890 / #1982 / #2066-#2081 | Split JSON-RPC request builders or repeated MCP payload assertion helpers before adding another tool family; keep host-facing compact/full/error contracts visible together while the MCP surface stabilizes. The 2026-06-17 sweep kept compact health-card defaults and recoverable agent-host action cards here because they guard one MCP host contract. `latest_reply` MCP closeout semantics now live in `tests/aippocampus/test_mcp_latest_reply.py`. |
| `tests/aippocampus/test_update_sync.py` | 2117 | 2180 | #1307 / #1335 / #1431 / #1563 / #1748 / #1755 / #1810 / #1982 / #2078 | Split Codex config/cache fixture builders, plugin apply/status assertion helpers, or dirty-worktree fixture setup before adding another host install/sync lane; keep one local-update contract suite until the marketplace/cache/current-thread visibility behavior stabilizes. The Windows hidden lifecycle launcher, plugin-cache smoke assertions, key-tool live host smoke, host-conformance label tests, and dirty-sync guard stay here because they exercise the same local install/update contract. |
| `tests/aippocampus/test_agent_opt_in_continuity.py` | 2066 | 2120 | #1507 / #1620 / #1808 / #1814 / #2069 | Split shared CLI/MCP fixture builders or returned-packet shape assertion helpers before adding another foreground recall surface; keep opt-in recall, AIppo, macro, compact/full recall boundary, and action-card JSON contracts visible together while the agent-facing packet shape is stabilizing. #2069 kept available-vs-global AIppo clause-count assertions here because they guard the same `agent aippo` JSON contract. |
| `tests/aippocampus/test_aippocampus_cli.py` | 2016 | 2080 | #1756 / #1843 / #1838 / #1845 / #1774-#1823 / #2066-#2078 | Split subprocess runner helpers or command-family smoke tests into focused modules before adding more top-level facade, personal-control, learning, or package-facade cases; keep the first-five-minutes path visible in one smoke owner. Command-family recovery/help contracts live in `tests/aippocampus/test_cli_recovery_cards.py`; only facade-owned first-use smoke should stay here. |
| `tests/aippocampus/test_aippocampus_health.py` | 1861 | 1920 | #248 / #1809 / #1782 / #1890 / #2081 | Split storage-pressure fixture builders, host-state fixture builders, or compact/full projection assertion helpers before adding another health lane; keep generated-cache pressure advisory, host-state confound separation, compact agent cards, and live-delta readiness together until that contract stabilizes. |
