...
| `skills/aippocampus/scripts/aippocampus_runtime/contracts.py` | 602 | 640 | P1 foreground-contract-risk | Split executable-command linting or readiness-card vocabulary before adding another foreground action family, public envelope, or command-shape rule. | #2412 keeps non-runnable command marker detection beside the shared foreground-action contract so no-op/continue cards cannot masquerade as shell commands. Do not add another cross-surface action lint, public envelope family, or readiness-card expansion here without extracting a focused contract helper. |
...
| `skills/aippocampus/scripts/aippocampus_runtime/mcp/server.py` | 1248 | 1260 | P1 agent-host-risk | Split tool-family handlers before adding another diagnostic surface, host-specific MCP projection, follow-up cache mutation, or write-like setup lane. | #1656-#1676 kept compact `agent_recall`/`recall_context`, request-index `agent_deepen` follow-through, and explicit `register_thread` write consent in the MCP facade because they close one existing foreground contract. #2081 kept compact health exposure here because it is the existing `memory_health` tool family. #2281 kept last-recall opened-route cache write/read wiring here because MCP recall/deepen must share the CLI's already-opened route semantics. #2341 kept selector-backed same-machine recall/deepen parity here because the MCP host-facing tools must understand the same callable selector as the CLI while cache policy and projection live in split helpers. #2389/#2393 kept `search_memory` scoped source-search parity and opt-in APW diagnostic argument wiring here because both extend existing MCP tool families; the next MCP expansion should extract handler families. |
...
| `skills/aippocampus/scripts/aippocampus_runtime/subconscious/scheduler.py` | 879 | 900 | P2 hook-scheduler-risk | Split agent-fallback enqueue wrapping, opt-in/consent policy, or detached worker command assembly before adding another scheduler mode, worker lane, or lifecycle policy. | #2269 kept explicit hook opt-in and background-model consent checks in the scheduler because it owns whether a detached model-backed worker may run at all. Extract opt-in policy or command assembly before adding another scheduler lane. |
...
| `skills/aippocampus/scripts/aippocampus_runtime/journey/live.py` | 635 | 660 | P2 journey-replay-risk | Split public replay fixture catalog or report projection into a sibling helper before adding another Journey replay source family, live-host adapter, or foreground policy branch. | #2372 moved compact `journeys.jsonl` sidecar materialization into `journey/sidecar_materializer.py`; `live.py` keeps live-row conversion, foreground hint timing, wrappers, and the small public replay cohort because they share the same no-write / no-raw-source / navigation-only boundary. The next expansion should extract the public replay cohort catalog or report projection instead of raising this budget again. |
...
| `tests/aippocampus/test_aippocampus_mcp_server.py` | 2471 | 2500 | #1450 / #1451 / #1656-#1676 / #1813 / #1817 / #1712 / #1889 / #1890 / #1982 / #2066-#2089 / #2013 / #2173 / #2174 / #2181 / #2203-#2226 / #2228-#2252 / #2389 / #2393 | Split JSON-RPC request builders, source-window privacy fixtures, tool-catalog assertion helpers, search-scope assertion helpers, or readiness-repair payload fixtures before adding another tool family; keep host-facing compact/full/error contracts visible together while the MCP surface stabilizes. The 2026-06-18 foreground sweep kept list/readiness/template-action checks here because they guard the same compact/default MCP contract. #2389/#2393 kept `search_memory` source-scope parity and opt-in APW diagnostics in this suite because both are existing MCP tool contracts. `latest_reply` MCP closeout semantics now live in `tests/aippocampus/test_mcp_latest_reply.py`. |
