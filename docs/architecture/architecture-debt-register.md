...
| `skills/aippocampus/scripts/aippocampus_runtime/hooks/claude_code.py` | 777 | 840 | P1 claude-hook-host-risk | Split settings mutation helpers, event-status projection, or handler-command resolution before adding another Claude Code event family or host-version lane. | #2010 keeps status, dry-run, explicit install/uninstall, synthetic smoke, and fail-open scoped handlers together because they guard the same Claude settings contract. PostToolUse/PostCompact support or real-host firing evidence should extract a focused owner instead of growing this file. |
| `skills/aippocampus/scripts/aippocampus_runtime/mcp/recall_navigation.py` | 1017 | 1040 | P1 agent-recall-risk | Split topic-label selection, source-window projection, or malformed-handle diagnostics before adding another route family, handle family, or freshness policy. | #1278 and #1188 deliberately kept display-vs-callable deepen diagnostics and fixed-vocabulary route-topic labels in the progressive MCP recall owner because they directly shape the same `recall_context -> recall_deepen` packet contract. #2026 kept latest-reply reopen in this owner because it is another handle-resolution branch, not a new recall surface. Do not add more route-label or freshness heuristics here without extracting a focused helper. |
| `skills/aippocampus/scripts/aippocampus_runtime/mcp/memory_health_recovery.py` | 355 | 380 | P1 memory-health-foreground-risk | Split recall-capability probing from public recovery-card projection before adding another health, maintenance, or host-readiness branch. | #2129 keeps exception recovery and compact health-card recall-first overlay together because both prevent MCP `memory_health` from hiding a source-backed `agent_recall -> agent_deepen` path. Do not add maintenance planning, indexing policy, or host install status here; keep those in their existing owners and only project the foreground split state. |
| `skills/aippocampus/scripts/aippocampus_runtime/mcp/agent_recall_projection.py` | 674 | 700 | P1 recall-projection-risk | Split APW fallback/policy compaction or route-receipt projection before adding another compact recall action family, posture field family, or low-confidence recovery lane. | #2483 kept APW semi-default recovery projection here because compact MCP/CLI recall must choose one foreground action and preserve ordinary recovery as secondary context. The next recall projection growth should extract APW policy/card projection or route receipt rendering instead of widening this mixed compact surface. |
| `skills/aippocampus/scripts/aippocampus_runtime/mcp/tool_handlers.py` | 1055 | 1120 | P1 mcp-tool-handler-risk | Split recall, source-search, health, or Telepathy handler families before adding another MCP tool family or write-like setup lane. | #2530 moved tool argument parsing, handler execution, and text/error result shaping out of `mcp/server.py` so the server stays JSON-RPC/stdio glue. This helper is the new MCP tool-family owner; do not let it become the next host facade by adding unrelated protocol behavior here. |
| `skills/aippocampus/scripts/aippocampus_runtime/recall/associative_path_inputs.py` | 665 | 700 | P1 apw-input-risk | Split current clean-source candidate derivation or source-reopen action projection before adding another APW input family. | #2525/#2527 added current clean-source APW diagnostic parity here because sidecar and clean-source candidates must share dedupe, privacy, and source-ref shaping before the walker runs. The next input source, scoring branch, or action projection should move into a focused helper instead of growing this bridge. |
...
| `skills/aippocampus/scripts/aippocampus_runtime/source/registry_search.py` | 625 | 640 | P1 exact-source-search-risk | Split last-search cache IO or bounded source-window projection before adding ranking, source-set resolution, or route-bridged search policy. | #2388/#2389 kept registry-wide source search, same-machine hit selectors, and last-recall scoped exact search in one owner because they share thread/message/line reopen semantics and privacy-safe cache rules. Keep raw registry-entry projection out of this source-search owner. |
| `skills/aippocampus/scripts/aippocampus_runtime/source/last_recall_search.py` | 608 | 640 | P1 last-recall-search-risk | Split selector/cache recovery or bounded source-window projection before adding another fallback lane, ranking gate, or frontend action family. | #2487/#2491/#2492 made low-specificity recall misses recover through source search and stable selectors. Keep stale-cache recovery, last-recall selector handling, and source-window reopening from becoming another recall router; extract one of those before further growth. |
...
| `skills/aippocampus/scripts/aippocampus_runtime/recall/prompt_cues.py` | 436 | 740 | P1 foreground-cue-risk | Future growth should split current-checkout, secret-surface, memory-boundary, or semantic-gate intent helpers only when one of those policy owners grows again. | #1145 split static cue term/pattern catalogs into `prompt_cue_catalog.py` while keeping legacy `prompt_cues` imports stable; `prompt_cues.py` now owns prompt intent, gating, and source-boundary helper behavior. |
...
| `skills/aippocampus/scripts/aippocampus_runtime/navigation/cognitive_map.py` | 617 | 660 | P2 cold-map-owner-risk | Split matcher memoization or map-file IO into a sibling helper before adding another cold-map scoring, publication, or foreground projection branch. | #2261 kept mtime-backed map loading and signature-based `match_cognitive_map` memoization beside the cognitive-map owner because the cache key must follow the map schema and matching semantics. Do not broaden this file into a general prompt-cache layer. |
...
| `skills/aippocampus/scripts/aippocampus_runtime/warm_ambient/recall.py` | 1343 | 1438 | P2 background-runtime-risk | Split batch/quorum bookkeeping or job/cache summary projection before adding more warm runtime orchestration. | Recent helper splits moved config, scout attribution, privacy action result policy, and topic-epoch write policy out; another extraction would be speculative unless the next warm-runtime growth touches batch/quorum bookkeeping or job/cache summary projection. Split one of those before any further budget raise. |
...
| `skills/aippocampus/scripts/aippocampus_runtime/recall/agent_continuity.py` | 1413 | 1500 | #2530 | Source-backed recall/deepen/explain behavior, AIppo/macro packet building, low-authority feedback capture, and source reopen semantics; parser/help/command dispatch now lives in `recall/agent_continuity_cli.py`. |
...
| `skills/aippocampus/scripts/aippocampus_runtime/ops/recall_navigation_promotion.py` | 687 | 740 | #2536 | Promotion metric/gate harness and feature-arm comparison; public smoke summary projection now lives in `ops/recall_navigation_promotion_projection.py`. |
...
| `tests/aippocampus/test_continuity_domains.py` | 1509 | 1750 | #973 / #926 | Split continuity-domain producer fixtures or route-projection assertions before adding another domain family, source-opening surface, or MCP handle contract. |
