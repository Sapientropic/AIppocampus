...
| `aippocampus_runtime/ops/route_readiness.py`, `aippocampus_runtime/ops/cognitive_observatory.py` | Public-safe, no-write route-readiness and Cognitive Observatory readouts. Route rows are navigation-only diagnostics with TTL/freshness/privacy/ROI suppression reasons; the observatory aggregates route readiness, activation authority, query-pattern route counts, recall diagnostics, and sleep-cycle summaries, and can render the sanitized report as static no-script HTML without becoming a control plane. | Public entrypoint |
