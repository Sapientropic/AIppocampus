...
| `aippocampus_runtime/macro/hexagram.py`, `aippocampus_runtime/macro/perturbation.py`, `aippocampus_runtime/macro/momentum.py`, `aippocampus_runtime/macro/state.py`, `aippocampus_runtime/macro/stage_tracker.py`, `aippocampus_runtime/macro/three_powers.py` | Deterministic hexagram substrate for macro-orientation experiments: exact King Wen lookup, bottom-to-top line convention, changing-line deltas, structural transforms, adjacency, distance bands, route-control perturbation amplitude, source-backed momentum phase compression, project-scoped macro-orientation state validation/projection, King Wen project-stage movement tracking, Three Powers route facets for earth/human/heaven fanout, and structure-only public projection. It must not carry line texts, model calls, interpretive advice, or project-level fanout/state mutation from unpromoted Journey/thread signals; perturbation, momentum, Three Powers facets, and stage updates are navigation metadata and do not replace masks, action grammar, or source-backed authority. | Runtime internal |
