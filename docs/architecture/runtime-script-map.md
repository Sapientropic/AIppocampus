...
| `aippocampus_runtime/macro/hexagram.py`, `aippocampus_runtime/macro/signal_scales.py`, `aippocampus_runtime/macro/perturbation.py`, `aippocampus_runtime/macro/momentum.py`, `aippocampus_runtime/macro/line_topology.py`, `aippocampus_runtime/macro/state.py`, `aippocampus_runtime/macro/stage_tracker.py`, `aippocampus_runtime/macro/three_powers.py`, `aippocampus_runtime/macro/audit.py` | Deterministic hexagram substrate for macro-orientation experiments: exact King Wen lookup, bottom-to-top line convention, changing-line deltas, structural transforms, adjacency, shared macro signal-scale normalization/classification, distance bands, route-control perturbation amplitude, source-backed momentum phase compression, diagnostic-only line topology, project-scoped macro-orientation state validation/projection, King Wen project-stage movement tracking, Three Powers route facets for earth/human/heaven fanout, and the #1272 Yi runtime-interface audit. It must not carry line texts, model calls, interpretive advice, or project-level fanout/state mutation from unpromoted Journey/thread signals; perturbation, momentum, line topology, Three Powers facets, stage updates, and audit fixtures are navigation metadata and do not replace masks, action grammar, or source-backed authority. | Runtime internal |
