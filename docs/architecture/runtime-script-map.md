...
| Script or group | Purpose | Invocation route | Key dependencies | Status |
|---|---|---|---|---|
| `subconscious_jobs.py`, `subconscious_jobs_config.py`, `subconscious_scheduler.py`, `subconscious_worker.py`, `question_tracking.py`, `journey_tracking.py`, `compensatory_dream.py`, `reflection_space.py` | Schedule and run background semantic/subconscious jobs, including deterministic Phase 2 question-link tracking, source-backed Journey P1-P3 helpers, review-only Phase 1 compensatory dream candidates, and reflection-space topology/feedback MVP helpers. | CLI, scheduler, opt-in maintenance. | Registry, model client, route metadata, job validation, source-backed question candidates, journey waypoints, extraction rows, dream candidate refs, reflection feedback rows, and topology source refs. | Public entrypoint |
