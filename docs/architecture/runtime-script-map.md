...
| `aippocampus_runtime/recall/agent_facade_contract.py` | Fixture-backed agent-native `recall` / `deepen` / `explain` projection contract over route packets. Foreground packets stay compact and source/provenance detail remains behind explicit deepen/explain calls. | Runtime internal |
