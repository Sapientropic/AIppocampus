...
| `aippocampus_runtime/ops/route_readiness.py`, `aippocampus_runtime/ops/cognitive_observatory.py`, `aippocampus_runtime/ops/observatory_cognitive_load.py` | Public-safe, no-write route-readiness and Cognitive Observatory readouts. Route rows are navigation-only diagnostics with TTL/freshness/privacy/ROI suppression reasons; the observatory aggregates route readiness, activation authority, query-pattern route counts, cognitive-load calibration summaries, recall diagnostics, and sleep-cycle summaries, and can render the sanitized report as static no-script HTML without becoming a control plane. | Public entrypoint |
...
| `aippocampus_runtime/navigation/project_timeline.py`, `aippocampus_runtime/navigation/associations.py`, `aippocampus_runtime/navigation/concept_graph.py`, `aippocampus_runtime/navigation/concept_graph_cli.py`, `aippocampus_runtime/navigation/concept_graph_schema.py`, `aippocampus_runtime/navigation/concept_lifecycle.py`, `aippocampus_runtime/navigation/concept_edge_utility.py`, `aippocampus_runtime/navigation/cognitive_map.py`, `aippocampus_runtime/navigation/cognitive_map_query.py`, `aippocampus_runtime/navigation/repo_familiarity.py` | Timeline, associations, concept/cognitive maps, explicit far/mid/near cognitive-map query packets, concept-graph CLI, concept-graph schema/migration helpers, concept-graph lifecycle policy, concept-edge utility telemetry, and repo familiarity navigation. Cognitive-map cold start may emit a registry-derived far-view overview when no source-backed route exists; that overview is navigation-only and must not be treated as a semantic route or claim evidence. | Rebuildable cache |
