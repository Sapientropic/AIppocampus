...
| `aippocampus_runtime/subconscious/` | `subconscious_job_plan.py`, `subconscious_job_storage.py`, `subconscious_jobs_config.py` | Sample-wave planning, append-only job-finding storage, and run configuration/default-path helpers used by subconscious job runners. |
...
| `subconscious_runtime.py`, `subconscious_agent.py`, `subconscious_tool_loop.py`, `aippocampus_runtime/subconscious/{job_plan,job_storage,jobs_config}.py` plus `subconscious_job_plan.py` / `subconscious_job_storage.py` / `subconscious_jobs_config.py` compatibility shims | Runtime loops, tool execution, sample-wave planning, append-only finding storage, and job-run configuration for background agents/jobs. | Scheduler/worker internals. | Job config, external model client, registry outputs. | Runtime internal |
