...
| `aippocampus_runtime/sync/encrypted/{admin,bundle,object_storage,crypto,keys,migration}.py` plus `encrypted_sync_*` compatibility shims | Age-backed encrypted sync overlay, device-key UX, and plaintext migration helpers. | CLI/encrypted sync docs; `encrypted_sync_admin.py` remains a direct compatibility command. | Sync bundle/object transport plus key handling. | Public entrypoint |
...
| `aippocampus_runtime/source/{semantic_scope_source_review_core,semantic_scope_suppressed_recovery}.py` plus `semantic_scope_source_review_core.py` / `semantic_scope_suppressed_recovery.py` compatibility shims | Source review and suppressed-label recovery flows. | Smoke/maintenance and optional pro-model jobs. | Clean source, model client, semantic route metadata. | Repo maintenance |
