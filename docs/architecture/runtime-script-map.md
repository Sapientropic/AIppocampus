...
| `aippocampus_runtime/artifacts/{export_bundle,import_bundle}.py` plus `export_bundle.py` / `import_bundle.py` compatibility shims, `checkpoint.py` | Move or checkpoint clean-source memory bundles. | `aippocampus export`, `aippocampus import`, and direct script compatibility paths for manual export/import; checkpoint remains a direct maintenance script. | Registry paths, clean-source manifests, privacy boundaries, package index-builder API. | Public entrypoint |
