...
| `aippocampus_runtime/dream/` | `dream_delivery_policy.py`, `dream_one_sidedness.py`, `dream_precision_policy.py`, `dream_retrospective_lifecycle.py`, `dream_sleep_cycle.py`, `dream_worker_contract.py` | Hook-safe dream delivery policy, deterministic one-sidedness gating, deterministic dream-hypothesis precision policies, retrospective lifecycle checks for parked future-facing probes, detached sleep-cycle queue execution, and stable bounded-worker prompt contract. |
