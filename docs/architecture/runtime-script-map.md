...
| Active path, drawer, facade, and narrative orientation | `tests/aippocampus/test_active_path_packet.py`, `tests/aippocampus/test_agent_native_recall_facade.py`, `tests/aippocampus/test_agent_pull_gesture.py`, `tests/aippocampus/test_memory_evidence_drawer.py`, `tests/aippocampus/test_narrative_packet.py` | Compact task-start path selection across existing recall/navigation surfaces, agent-native recall/deepen/explain packets, the copyable agent-pull gesture, opt-in evidence drawer projection, source-joined route notes, task-time narrative packets that merge pathlets/domain pointers/sequence packets/active routes/glyphs, route separation for scent/reopen/evidence/ignore, stale-boundary visibility, ids-only source refs, and navigation-only cues blocked from masquerading as factual evidence. |
