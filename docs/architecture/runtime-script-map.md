...
| Script or group | Purpose | Invocation route | Key dependencies | Status |
|---|---|---|---|---|
| `subconscious_jobs.py`, `subconscious_jobs_config.py`, `subconscious_scheduler.py`, `subconscious_worker.py`, `question_tracking.py`, `journey_tracking.py`, `compensatory_dream.py`, `dream_input_pack.py`, `dream_real_history_eval.py`, `dream_working_memory.py`, `reflection_space.py` | Schedule and run background semantic/subconscious jobs, including deterministic Phase 2 question-link tracking, source-backed Journey P1-P3 helpers, Phase 1 compensatory dream candidates, P2 cross-thread dream input packs, selected real-history structural dream eval, background-adjudicated dream-hypothesis projection to working memory, and reflection-space topology/feedback MVP helpers. | CLI, scheduler, opt-in maintenance. | Registry, model client, route metadata, job validation, source-backed question candidates with source-derived scope labels, journey waypoints, extraction rows, question links, Journey rows, working-memory rows, ambient residue handles, dream input packs, dream candidate refs, adjudicated dream source refs, reflection feedback rows, and topology source refs. | Public entrypoint |
