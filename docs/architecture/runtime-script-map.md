...
| `aippocampus_runtime/subconscious/` | `memory_candidate_router.py`, `question_resolution.py`, `subconscious_agent.py`, `subconscious_deterministic_jobs.py`, `edge_validation.py`, `shell_selection.py`, `subconscious_job_circuits.py`, `subconscious_jobs.py`, `subconscious_jobs_config.py`, `subconscious_question_diagnostics.py`, `subconscious_runtime.py`, `subconscious_scheduler.py`, `subconscious_staging_maintenance.py`, `subconscious_worker.py`, `theme_emergence.py` | Candidate routing and hook-safe foreground stripping, minimal read-only tool-loop agent runner, deterministic follow-up dispatch, deterministic question-resolution/theme-emergence runners, shared concept-edge validation policy for worker/agent staging rows, dry-run shell selection cost/quality reports, job circuit/schema contracts, package-only sample-wave planning, package-only append-only job-finding storage, package-only finding validation, default dry-run staging queue maintenance/backpressure reporting plus explicit operator apply-mode archive manifests, model/deterministic job runner orchestration, run configuration/default-path helpers, question-extraction diagnostics, shared read-only runtime primitives, hook-safe detached scheduler, package-owner shared tool/final loop helpers, package-only validation diagnostics, and single-shot consolidation worker helpers used by subconscious job runners. |
