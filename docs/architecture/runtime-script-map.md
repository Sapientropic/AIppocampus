...
| `aippocampus_runtime/ops/` | `storage_capacity_report.py`, `rollout_size_audit.py`, `retention_report.py`, `cold_archive.py`, `recall_funnel_smoke.py`, `storage_governance.py` | Operator-facing storage, retention, raw-rollout, progressive recall funnel, and storage-governance diagnostics; outside foreground recall. |
