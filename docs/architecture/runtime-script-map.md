...
| `aippocampus_runtime/dream/` | `dream_delivery_policy.py`, `dream_one_sidedness.py`, `dream_precision_policy.py`, `dream_retrospective_lifecycle.py`, `dream_worker_contract.py` | Hook-safe dream delivery policy, deterministic one-sidedness gating, deterministic dream-hypothesis precision policies, retrospective lifecycle checks for parked future-facing probes, and stable bounded-worker prompt contract. |
...
| `prompt_cues.py`, `aippocampus_runtime/recall/`, `prompt_life_cues.py` compatibility shim, `prompt_context_render.py`, `prompt_recall_ambient.py`, `prompt_recall_budget.py`, `prompt_recall_context.py`, `prompt_recall_core.py`, `prompt_recall_decision.py`, `prompt_recall_evidence.py` | Prompt recall policy, life-wide/profile cue catalogs, budget, rendering, query policy, and source-evidence assembly. | Called by prompt hook, active recall, and smokes. | Registry/search, timeline, semantic cue cache. | Runtime internal |
