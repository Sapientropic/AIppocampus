...
| `aippocampus_runtime/navigation/project_timeline.py`, `aippocampus_runtime/navigation/associations.py`, `aippocampus_runtime/navigation/concept_graph.py`, `aippocampus_runtime/navigation/concept_graph_cli.py`, `aippocampus_runtime/navigation/concept_graph_schema.py`, `aippocampus_runtime/navigation/concept_lifecycle.py`, `aippocampus_runtime/navigation/concept_edge_utility.py`, `aippocampus_runtime/navigation/cognitive_map.py`, `aippocampus_runtime/navigation/cognitive_map_query.py`, `aippocampus_runtime/navigation/repo_familiarity.py`, `aippocampus_runtime/navigation/attention_route_tokens.py`, `aippocampus_runtime/navigation/attention_router_contract.py` | Timeline, associations, concept/cognitive maps, explicit far/mid/near cognitive-map query packets, concept-graph CLI, concept-graph schema/migration helpers, concept-graph lifecycle policy, concept-edge utility telemetry, repo familiarity navigation, source span/event/episode route-token projection, and the source-backed attention-router hard-mask / route-packet contract. Cognitive-map cold start may emit a registry-derived far-view overview when no source-backed route exists; that overview is navigation-only and must not be treated as a semantic route or claim evidence. Attention route packets and route tokens are also navigation-only unless source is reopened or already source-open within scope. | Rebuildable cache |
