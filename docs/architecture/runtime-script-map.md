...
| `aippocampus_runtime/recall/agent_facade_contract.py`, `aippocampus_runtime/recall/cross_agent_isolation.py` | Fixture-backed agent-native `recall` / `deepen` / `explain` projection contract over route packets, plus cross-agent read-path isolation hard negatives. Foreground packets stay compact, source/provenance detail remains behind explicit deepen/explain calls, and scope filters apply before search/recall/deepen/cache/semantic outputs. | Runtime internal |
...
| Active path, drawer, and narrative orientation | `tests/aippocampus/test_active_path_packet.py`, `tests/aippocampus/test_memory_evidence_drawer.py`, `tests/aippocampus/test_narrative_packet.py` | Compact task-start path selection across existing recall/navigation surfaces, opt-in evidence drawer projection, source-joined route notes, task-time narrative packets that merge pathlets/domain pointers/sequence packets/active routes/glyphs, route separation for scent/reopen/evidence/ignore, stale-boundary visibility, ids-only source refs, and navigation-only cues blocked from masquerading as factual evidence. |
