...
| `prompt_cues.py`, `aippocampus_runtime/recall/life_cues.py`, `prompt_life_cues.py` compatibility shim, `prompt_context_render.py`, `prompt_recall_ambient.py`, `prompt_recall_budget.py`, `prompt_recall_context.py`, `prompt_recall_core.py`, `prompt_recall_decision.py`, `prompt_recall_evidence.py` | Prompt recall policy, life-wide/profile cue catalogs, budget, rendering, and source-evidence assembly. | Called by prompt hook, active recall, and smokes. | Registry/search, timeline, semantic cue cache. | Runtime internal |
