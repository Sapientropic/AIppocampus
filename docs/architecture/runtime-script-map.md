...
| `aippocampus_runtime/recall/agent_facade_contract.py`, `aippocampus_runtime/recall/agent_pull_gesture.py`, `aippocampus_runtime/recall/cross_agent_isolation.py` | Fixture-backed agent-native `recall` / `deepen` / `explain` projection contract over route packets, the copyable `source_backed_continuity_gesture_v1` agent-pull workflow, plus cross-agent read-path isolation hard negatives. Foreground packets stay compact, source/provenance detail remains behind explicit deepen/explain calls, and scope filters apply before search/recall/deepen/cache/semantic outputs. | Runtime internal |
