...
| `aippocampus_runtime/macro/hexagram.py`, `aippocampus_runtime/macro/perturbation.py`, `aippocampus_runtime/macro/state.py`, `aippocampus_runtime/macro/three_powers.py` | Deterministic hexagram substrate for macro-orientation experiments: exact King Wen lookup, bottom-to-top line convention, changing-line deltas, structural transforms, adjacency, distance bands, route-control perturbation amplitude, project-scoped macro-orientation state validation/projection, Three Powers route facets for earth/human/heaven fanout, and structure-only public projection. It must not carry line texts, model calls, interpretive advice, or project-level fanout/state mutation from unpromoted Journey/thread signals; Three Powers facets are navigation metadata and do not replace masks, action grammar, or source-backed authority. | Runtime internal |
