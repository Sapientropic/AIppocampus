...
| `aippocampus_runtime/{text,safety,anchor_graph,schema_profiles}.py` and `aippocampus_runtime/cli/errors.py` | none | Direct owners for compact text helpers, external-model redaction/HTTP credential transport/cache metric helpers, thread-anchor parsing/graph construction, schema profile projection/validation, and CLI error payload shaping. These are package APIs, not subprocess-backed facades. |
| `aippocampus_runtime/health.py`, `aippocampus_runtime/health_registry.py`, `aippocampus_runtime/health_render.py` | `aippocampus_health.py` | Runtime readiness remains in `health.py`; privacy-safe registry-wide aggregation lives in `health_registry.py`; text projection lives in `health_render.py`. The public compatibility shim still exposes the same CLI/API surface for CLI, MCP, and frozen binaries without subprocess script dispatch. |
...
| `aippocampus_runtime/ops/` | `storage_capacity_report.py`, `rollout_size_audit.py`, `retention_report.py`, `cold_archive.py`, `provider_doctor.py`, `recall_funnel_smoke.py`, `recall_navigation_comparison.py`, `recall_navigation_comparison_fixtures.py`, `repo_familiarity_foreground_experiment.py`, `repo_familiarity_foreground_experiment_fixtures.py`, `storage_governance.py`, `storage_eviction.py`, `activation_authority_audit.py` | Operator-facing storage, retention, raw-rollout, live-provider visibility, progressive recall funnel, deterministic recall-navigation arm comparison and its public-safe fixture owner, opt-in repo-familiarity foreground experiment evidence, storage-governance, storage-eviction apply helpers, and activation-surface authority diagnostics; outside foreground recall. `activation_authority_audit.py` also owns the #483 no-write foreground-usefulness metrics and append-only lifecycle apply manifest for strategy-like activation rows. |
| `aippocampus_runtime/question/` | `question_health.py`, `question_tracking.py` | Borderline confirmation artifact helpers now live package-owner-only; derived question lifecycle health stats and deterministic question-link tracking remain direct compatibility shims; feedback pressure, source-ref resolution, and optional local vector-index hinting are package-owner helpers. |
...
| `aippocampus_runtime/subconscious/` | `memory_candidate_router.py`, `question_resolution.py`, `subconscious_jobs.py`, `subconscious_jobs_config.py`, `subconscious_runtime.py`, `subconscious_scheduler.py`, `subconscious_staging_maintenance.py`, `subconscious_worker.py`, `theme_emergence.py` | Candidate routing and hook-safe foreground stripping, package-owner minimal read-only tool-loop agent runner, deterministic follow-up dispatch, deterministic question-resolution/theme-emergence runners, shared concept-edge validation policy for worker/agent staging rows, dry-run shell selection cost/quality reports, package-owner job circuit/schema contracts, package-only sample-wave planning, package-only append-only job-finding storage, package-only finding validation, default dry-run staging queue maintenance/backpressure reporting plus explicit operator apply-mode archive manifests, model/deterministic job runner orchestration, run configuration/default-path helpers, package-owner question-extraction diagnostics, shared read-only runtime primitives, hook-safe detached scheduler, package-owner shared tool/final loop helpers, package-only validation diagnostics, and single-shot consolidation worker helpers used by subconscious job runners. |
...
`aippocampus_runtime/recall/living_cue_cache.py` belongs to the same recall
package owner row above. It is listed separately here because it defines a new
#281 generated-sidecar boundary: local digested cue entries may carry cue text
and aliases, but selector packets and reports expose only cue ids, source
handles, support levels, and count-only diagnostics.
...
| `tools/aippocampus/smoke/smoke_living_cue_cache.py` | Public-safe #281 living cue cache smoke for learned-phrase source-handle bridging and temporary-mood suppression, with count-only diagnostics. | Manual fresh-thread recall boundary smoke and targeted PR verification. | `aippocampus_runtime.recall.living_cue_cache`; public-safe fixtures only. | Repo maintenance |
...
| `aippocampus_runtime/health.py`, `aippocampus_runtime/health_registry.py`, `aippocampus_runtime/health_render.py`, and `aippocampus_health.py` compatibility shim | Runtime readiness, public smoke checks, optional derived question stats, registry-wide health rollups, and text rendering. | CLI, install docs, CI-adjacent smoke, MCP `memory_health`, active recall, registry registration, and frozen binaries. | `HealthOptions`, `build_health_report()`, and `health_report()` are the package API; CLI parsing stays at the edge. `health_registry.py` owns privacy-safe registry aggregation without reading raw/clean-source bodies; `health_render.py` owns human text projection so JSON report shape stays stable. Optional question stats are lazy-loaded so basic health does not pull the question/registry/subconscious graph at import time. | Public entrypoint |
...
| `aippocampus_runtime/recall/prompt_*`, `aippocampus_runtime/recall/semantic_*`, and remaining top-level compatibility shims (`semantic_recall_gate.py`, `semantic_trigger_router.py`) | Prompt recall cue policy, life-wide/profile cue catalogs, named skip/scent/evidence gate policy, final source-evidence projection, context-aware scent-threshold diagnostics, ambient cards/cache/policy, budget, rendering, query policy, source-evidence assembly, optional semantic gate/result-cache/cue-cache, and trigger routing. Deleted cue/budget/core/evidence/cache/render/decision helper shims now resolve through package owners only. | Called by prompt hook, active recall, scheduler, and smokes. | Registry/search, timeline, semantic cue cache, model route metadata. | Runtime internal |
...
| `aippocampus_runtime/recall/{retrieval,query_policy,structure_time,result_diversity,scoring_policy,score_fusion,rollout_search}.py`, `retrieval.py` / `retrieval_query_policy.py` / `retrieval_score_fusion.py` / `search_rollout.py` compatibility shims | Source-backed retrieval, D5/D6 structure/time cue lanes, result diversity ordering, named scoring policies, explicit score-fusion behavior, and raw audit fallback. | Prompt hook, MCP, CLI search surfaces, active recall, segmented retrieval, future vector/graph fusion consumers. | Clean-source JSONL, registry rows, query policy, `message_features` sidecar, stable source ids/source refs; scores remain ranking hints only. `scoring_policy.py` owns retrieval/segment/active-recall ranking weights; `prompt_recall_policy.py` owns foreground skip/scent/evidence thresholds and source-evidence quality boosts; neither module should replace semantic/subconscious judgement with static cue wordlists. | Runtime internal |
...
| `aippocampus_runtime/question/{confirmation,tracking,prefilter_report}.py` plus `question_tracking.py` compatibility shim | Deterministic question-link tracking, prefilter-mode observability, plus package-owner-only borderline confirmation artifact parsing and audit diagnostics. | Deterministic job runner, manual CLI, and future live calibration; `question_tracking.py` remains a direct compatibility path. | Pair ids, compact source-backed question payloads, source refs, salience/threshold policy; no full-history model input. | Runtime internal |
