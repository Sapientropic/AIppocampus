...
| `aippocampus_runtime/health.py` and `aippocampus_health.py` compatibility shim | Runtime readiness, public smoke checks, and optional derived question stats. | CLI, install docs, CI-adjacent smoke, MCP `memory_health`, and frozen binaries. | `aippocampus_runtime/registry/api.py`, `aippocampus_runtime/core.py`, `aippocampus_runtime/question/health.py`, filesystem/env checks. | Public entrypoint |
