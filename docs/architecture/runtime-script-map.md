...
| `aippocampus_runtime/ops/route_readiness.py`, `aippocampus_runtime/ops/cognitive_observatory.py` | Public-safe, no-write route-readiness and Cognitive Observatory readouts. Route rows are navigation-only diagnostics with TTL/freshness/privacy/ROI suppression reasons; the observatory aggregates route readiness, activation authority, recall diagnostics, and sleep-cycle summaries without becoming a control plane. | Public entrypoint |
...
| `aippocampus_runtime/recall/active_recall.py`, `aippocampus_runtime/recall/active_recall_lock.py`, `aippocampus_runtime/recall/active_recall_lock_compaction.py`, `aippocampus_runtime/recall/active_path_packet.py`, `aippocampus_runtime/recall/ambient_cards.py`, `aippocampus_runtime/recall/ambient_cache.py`, `aippocampus_runtime/recall/ambient_cache_compaction.py`, `aippocampus_runtime/recall/ambient_policy.py` | Foreground active-recall, active-path orientation packet, and ambient-card state, plus active-lock and ambient-cache dead-letter compaction as maintenance apply work. | Runtime internal |
...
| Active path orientation | `tests/aippocampus/test_active_path_packet.py` | Compact task-start path selection across existing recall/navigation surfaces, route separation for scent/reopen/evidence/ignore, stale-boundary visibility, and ids-only source refs. |
