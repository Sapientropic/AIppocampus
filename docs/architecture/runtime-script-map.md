...
| `aippocampus_runtime/macro/hexagram.py`, `aippocampus_runtime/macro/perturbation.py`, `aippocampus_runtime/macro/state.py` | Deterministic hexagram substrate for macro-orientation experiments: exact King Wen lookup, bottom-to-top line convention, changing-line deltas, structural transforms, adjacency, distance bands, route-control perturbation amplitude, project-scoped macro-orientation state validation/projection, and structure-only public projection. It must not carry line texts, model calls, interpretive advice, or project-level fanout/state mutation from unpromoted Journey/thread signals. | Runtime internal |
