...
| `aippocampus_runtime/update/cli.py`, `aippocampus_runtime/update/agent_callable.py`, `aippocampus_runtime/update/plugin_cache.py` | Local update status/plan/apply facade, foreground host tool-visibility projection, and Codex plugin marketplace/cache freshness diagnostics. The plugin cache helper keeps repo-local package output, local marketplace copies, and installed Codex cache copies distinct; cache refresh is explicit and may still need host reload evidence before any agent-callable claim. | Public entrypoint |
