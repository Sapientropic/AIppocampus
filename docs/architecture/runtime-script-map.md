...
| `aippocampus_runtime/aippo/working_contract.py` | Fixture-backed AIppo working-contract compiler output: clause-level lifecycle, compact activation packets, and deepen/explain audit surfaces over public-safe source trails. Candidate surfaces may nominate or route clauses, but only source/path support can make a clause ripe. | Runtime internal |
...
| `aippocampus_runtime/ops/route_readiness.py`, `aippocampus_runtime/ops/cognitive_observatory.py`, `aippocampus_runtime/ops/observatory_cognitive_load.py` | Public-safe, no-write route-readiness and Cognitive Observatory readouts. Route rows are navigation-only diagnostics with TTL/freshness/privacy/ROI suppression reasons, including stale/superseded/conflicted/refuted/uncertain route states; the observatory aggregates route readiness, activation authority, query-pattern route counts, cognitive-load calibration summaries, recall diagnostics, and sleep-cycle summaries, and can render the sanitized report as static no-script HTML without becoming a control plane. | Public entrypoint |
...
| `aippocampus_runtime/navigation/project_timeline.py`, `aippocampus_runtime/navigation/associations.py`, `aippocampus_runtime/navigation/concept_graph.py`, `aippocampus_runtime/navigation/concept_graph_cli.py`, `aippocampus_runtime/navigation/concept_graph_schema.py`, `aippocampus_runtime/navigation/concept_lifecycle.py`, `aippocampus_runtime/navigation/concept_edge_utility.py`, `aippocampus_runtime/navigation/cognitive_map.py`, `aippocampus_runtime/navigation/cognitive_map_query.py`, `aippocampus_runtime/navigation/repo_familiarity.py`, `aippocampus_runtime/navigation/attention_route_tokens.py`, `aippocampus_runtime/navigation/attention_hot_router.py`, `aippocampus_runtime/navigation/attention_evidence_packager.py`, `aippocampus_runtime/navigation/attention_router_contract.py`, `aippocampus_runtime/navigation/macro_router_interface.py`, `aippocampus_runtime/navigation/semantic_warm_route_producer.py` | Timeline, associations, concept/cognitive maps, explicit far/mid/near cognitive-map query packets, concept-graph CLI, concept-graph schema/migration helpers, concept-graph lifecycle policy, concept-edge utility telemetry, repo familiarity navigation, source span/event/episode route-token projection, deterministic V0 hot-router scoring with public-safe action-query extraction, source-window to source-span evidence packaging diagnostics, the source-backed attention-router hard-mask / route-packet contract, navigation-only Macro Orientation / router interface projection, and semantic-warming route material projection for background/cache-produced scout output. Cognitive-map cold start may emit a registry-derived far-view overview when no source-backed route exists; that overview is navigation-only and must not be treated as a semantic route or claim evidence. Attention route packets, route tokens, macro-router context/observations, and semantic-warm route material are navigation-only unless source is reopened or already source-open within scope. Evidence packaging may tighten handles and record currentness/conflict/counter-evidence requirements, but packaging scores, macro orientation, and semantic route scores are not source truth. | Rebuildable cache |
...
| Active path, drawer, facade, AIppo, and narrative orientation | `tests/aippocampus/test_active_path_packet.py`, `tests/aippocampus/test_agent_native_recall_facade.py`, `tests/aippocampus/test_agent_pull_gesture.py`, `tests/aippocampus/test_aippo_working_contract.py`, `tests/aippocampus/test_memory_evidence_drawer.py`, `tests/aippocampus/test_narrative_packet.py` | Compact task-start path selection across existing recall/navigation surfaces, agent-native recall/deepen/explain packets, the copyable agent-pull gesture, clause-level AIppo working-contract activation/deepen boundaries, opt-in evidence drawer projection, source-joined route notes, task-time narrative packets that merge pathlets/domain pointers/sequence packets/active routes/glyphs, route separation for scent/reopen/evidence/ignore, stale-boundary visibility, ids-only source refs, and navigation-only cues blocked from masquerading as factual evidence. |
