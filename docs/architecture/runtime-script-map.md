...
| `aippocampus_runtime/subconscious/` | `subconscious_job_storage.py` | Append-only job-finding storage helpers used by subconscious job runners. |
...
| `subconscious_runtime.py`, `subconscious_agent.py`, `subconscious_tool_loop.py`, `aippocampus_runtime/subconscious/job_storage.py` plus `subconscious_job_storage.py` compatibility shim | Runtime loops, tool execution, and append-only finding storage for background agents/jobs. | Scheduler/worker internals. | Job config, external model client, registry outputs. | Runtime internal |
