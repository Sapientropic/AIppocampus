...
Current snapshot after the #305 prompt-helper deletion batch:
...
| `delete_now` | 4 | Pure package-owner shims with no direct dependency left except, in most cases, their `py-modules` exposure; delete in small batches with the matching packaging entry. |
...
This file is only a short human pointer. Do not mirror the full 108-script list
