...
| `delete_now` | 25 | Pure package-owner shims with no direct dependency left except, in most cases, their `py-modules` exposure; delete in small batches with the matching packaging entry. |
...
This file is only a short human pointer. Do not mirror the full 125-script list
...
there is a public/operator contract until the skill text is migrated.
