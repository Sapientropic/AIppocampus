...
| `aippocampus_runtime/health_stages.py` | Codex-current-thread health stage owner | Owns health local-context and source-index freshness resolution, including Codex rollout discovery for current live thread freshness. Keep this classified with health instead of treating the split helper as provider-neutral. |
...
| `aippocampus_runtime/hooks/install_action_hint.py` | Codex host integration | Mutates Codex `PreToolUse` hook config only after explicit operator command; unsupported hosts report unsupported instead of pretending action-time hints are installed. |
...
| `aippocampus_runtime/ops/recall_funnel_live_agent_gate.py` | Codex-aware foreground diagnostic | Runs packaged `agent recall -> agent deepen` through a temporary same-machine selector cache, then reads Codex-home hook/update status only to summarize ambient readiness. It reports public-safe route/source/task-usefulness gates and must not expose cue text, source text, raw handles, or local paths. |
| `aippocampus_runtime/update/cli.py`, `aippocampus_runtime/update/status_foreground.py`, `aippocampus_runtime/update/agent_callable.py`, `aippocampus_runtime/update/plugin_cache.py`, `aippocampus_runtime/update/codex_plugin_cli.py`, `aippocampus_runtime/update/plugin_marketplace.py`, `aippocampus_runtime/update/plugin_uninstall_preview.py`, `aippocampus_runtime/update/plugin_installer.py` | Codex-aware local update facade, fast foreground status card, sibling readiness helpers, Codex plugin CLI compatibility helpers, AIppocampus-owned marketplace helpers, rollback preview, and Codex local plugin installer | `aippocampus update status/plan/apply` compares the repo skill, installed Codex skill copy, plugin staging, MCP config, hook config, provider-key visibility, foreground host exposure, and optional local marketplace / installed-cache plugin layers. Foreground `--agent-json` status may read Codex-home host probe state but defers slow local freshness and provider-child probes to `--operator-json`. Mutating update/apply paths may explicitly sync the installable skill, rebuild the repo-local plugin package, refresh plugin cache layers when the operator supplies cache paths, and install AIppocampus-owned Codex hooks. `aippocampus plugin install --codex --verify` owns the local Codex path: build package, refresh the AIppocampus-owned local marketplace and current Desktop versioned installed cache, tolerate already-registered local marketplace state, reload MCP, and call `sync_status`; `plugin uninstall --codex --dry-run` reports the same local rollback boundary without mutation. These surfaces do not enable hooks, mutate private memory artifacts, or read API-key values. |
| `aippocampus_runtime/ops/uninstall.py` | Cross-host local artifact inventory and explicit purge | `aippocampus uninstall --dry-run` inventories AIppocampus-owned Codex plugin/cache/skill/hook artifacts and separates registry user data. `--purge` removes host wiring only unless `--confirm-user-data` is explicit; it may call Claude Code uninstall helpers but does not read transcript contents or provider credentials. |
| `aippocampus_runtime/ops/doctors/provider_doctor.py` | Operator provider visibility and credential-source diagnostic | Normal `aippocampus doctor provider` is presence-only and reads no key values. The explicit `--discover-credential-sources` path may inspect the current process env and user-specified `.env` files to report redacted candidate shape and optional validation status; it does not scan by default, read OS credential stores, install hook wrappers, or change runtime provider visibility. `aippocampus_runtime/ops/provider_doctor.py` is only the public compatibility wrapper. |
| `aippocampus_runtime/ops/doctors/provider_credentials.py` | Explicit credential-source helper for provider doctor | Parses only user-specified `.env` files or current-process env under the provider doctor discovery flag. It owns redaction, optional HTTPS/loopback validation, and bridge-plan diagnostics; it is not a runtime credential loader or secret-store adapter. |
...
| --- | --- | --- | --- | --- | --- | --- |
| `codex` | Yes: Codex rollout provider. | Yes: clean source, registry, MCP, onboarding, and current-thread maintenance paths. | Yes: `aippocampus_runtime/hooks/prompt.py`, `aippocampus_runtime/hooks/lifecycle.py`, and the prepared-cache `PreToolUse` reader in `aippocampus_runtime/hooks/action_hint.py` implement Codex hook handlers. | Yes: `install_prompt.py`, `install_lifecycle.py`, and `install_action_hint.py` mutate Codex `hooks.json` after explicit operator command. | Yes: `diagnose.py` inspects Codex `hooks.json` and emulates Codex hook stdin. | Codex hook install/status/uninstall remains opt-in Codex host integration, not a provider-neutral AIppocampus install. |
| `claude-code` | Yes: Claude Code transcript parser and explicit onboarding. | Yes: clean-source registration and MCP/project-skill surfaces. | Scoped: `aippocampus_runtime/hooks/claude_code.py` handles `UserPromptSubmit` and `Stop` fail-open, with synthetic smoke evidence. | Yes: `aippocampus hooks claude-code install|uninstall --json` mutates only AIppocampus-owned `UserPromptSubmit` / `Stop` handler entries after explicit operator command and preserves unrelated settings. | Scoped status/dry-run/install/uninstall/smoke; real-host firing still needs an event log or local dogfood report. | Claude Code hook contract status: scoped handler/status/dry-run/install/uninstall/smoke support exists for `UserPromptSubmit` and `Stop`; real-host firing, `PostToolUse`/`PostToolBatch`, and compaction hook utility remain cannot-claim boundaries. |
