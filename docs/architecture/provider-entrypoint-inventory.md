...
| `aippocampus_runtime/update/cli.py`, `aippocampus_runtime/update/agent_callable.py`, `aippocampus_runtime/update/plugin_cache.py` | Codex-aware local update facade and sibling readiness helpers | `aippocampus update status/plan/apply` compares the repo skill, installed Codex skill copy, plugin staging, MCP config, hook config, provider-key visibility, foreground host exposure, and optional local marketplace / installed-cache plugin layers. It may explicitly sync the installable skill, rebuild the repo-local plugin package, refresh plugin cache layers when the operator supplies cache paths, and install AIppocampus-owned Codex hooks; it does not mutate private memory artifacts or read API-key values. |
