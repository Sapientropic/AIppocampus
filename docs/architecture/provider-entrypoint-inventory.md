...
| `aippocampus_runtime/update/cli.py`, `aippocampus_runtime/update/agent_callable.py`, `aippocampus_runtime/update/plugin_cache.py`, `aippocampus_runtime/update/codex_plugin_cli.py`, `aippocampus_runtime/update/plugin_installer.py` | Codex-aware local update facade, sibling readiness helpers, Codex plugin CLI compatibility helpers, and Codex local plugin installer | `aippocampus update status/plan/apply` compares the repo skill, installed Codex skill copy, plugin staging, MCP config, hook config, provider-key visibility, foreground host exposure, and optional local marketplace / installed-cache plugin layers. It may explicitly sync the installable skill, rebuild the repo-local plugin package, refresh plugin cache layers when the operator supplies cache paths, and install AIppocampus-owned Codex hooks. `aippocampus plugin install --codex --verify` owns the local Codex path: build package, refresh the AIppocampus-owned local marketplace and current Desktop versioned installed cache, tolerate already-registered local marketplace state, reload MCP, and call `sync_status`; it does not enable hooks, mutate private memory artifacts, or read API-key values. |
