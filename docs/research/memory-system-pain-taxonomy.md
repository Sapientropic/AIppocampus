...
| Eager LLM extraction cost and scale | Graphiti #1516, #1262, #1275, #1193 | Designed and partly implemented: registry/segment fanout and capacity reports avoid foreground LLM-on-write by default. | Scale fixtures should model large canonical documents without requiring eager graph extraction. |
| Invalid structured extraction | Graphiti #760 | Designed boundary: semantic sidecars and model findings are advisory until source-backed. | Fixtures should include plausible but unsupported structured findings that must be downgraded or reported. |
