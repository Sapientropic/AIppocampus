...
| `PreCompact` | Preserve continuity before context is rewritten. | Flush open correction windows, active task anchors, source refs, and recent outcome evidence so compaction cannot erase them. | Late semantic judgment or large rebuilds under compaction pressure. |
| `PostCompact` | Rehydrate continuity after context loss. | Re-inject high-value anchors only when visibility changed; mark horizon loss for Track D style checks. | Repeating still-visible context or treating the compacted summary as authoritative source. |
