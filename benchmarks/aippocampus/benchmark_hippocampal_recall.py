...
def _h5_after_response(case: Mapping[str, Any], arm: str) -> dict[str, Any]:
    if arm == "no_consolidation":
    ...
    payload = _with_adapter_metadata(
        response,
        arm=arm,
        candidate_count=len(_as_list(case.get("candidate_source_refs"))),
    )
...
def _h5_case_delta(case: Mapping[str, Any], arm: str) -> dict[str, Any]:
    before_response = run_adapter_arm(case, H5_BEFORE_ARM)
    after_response = _h5_after_response(case, arm)
...
def _h5_cost_totals(deltas: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for delta in deltas:
        ...
        for key in ("work_units", "model_calls", "tokens", "association_edges_created", "summary_units"):
...
def _h5_consolidation_report(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    case_deltas = [
        _h5_case_delta(case, arm)
