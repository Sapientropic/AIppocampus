...
class AippocampusCliTests(unittest.TestCase):
    ...
    def test_config_alias_recovers_to_safe_doctor(self) -> None:
        ...
        full_proc = self.run_cli("config", "--detail", "full")
        ...
        self.assertIn("use `aippocampus config --detail full`", human_proc.stdout)
        self.assertNotIn("AIPPOCAMPUS_PROMPT_HOOK_BUDGET_MS", human_proc.stdout)
        self.assertEqual(full_proc.returncode, 0, full_proc.stderr)
        self.assertIn("AIPPOCAMPUS_PROMPT_HOOK_BUDGET_MS", full_proc.stdout)
    ...
    def test_status_json_is_foreground_card_and_operator_json_is_full_detail(self) -> None:
        ...
        self.assertNotIn("foreground_status_cards", update_payload)
    ...
    def test_warm_status_human_keeps_optional_queue_from_blocking_recall(self) -> None:
        ...
        self.assertIn("next: Continue with ordinary recall", human.stdout)
        self.assertIn('action: aippocampus agent recall "{cue}" --json', human.stdout)
        self.assertNotIn("action: None", human.stdout)
