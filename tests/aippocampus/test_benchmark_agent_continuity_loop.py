...
class AgentContinuityLoopBenchmarkTests(unittest.TestCase):
    def test_public_safe_loop_composes_router_facade_aippo_and_budgets(self) -> None:
        ...
        self.assertIn("8/8 public-safe agent-continuity fixture cases passed", report["measured_result"])
        self.assertGreaterEqual(len(report["supports"]), 3)
        self.assertLessEqual(len(report["material_limits"]), 3)
        self.assertLessEqual(len(report["cannot_claim"]), 4)
        self.assertNotIn("public_benchmark_quality_lift", report["cannot_claim"])
        self.assertNotIn("external_model_quality", report["cannot_claim"])
