...
class MacroAgentPacketTests(unittest.TestCase):
    def test_agent_macro_orientation_returns_compact_navigation_packet(self) -> None:
        ...
        self.assertIn("initial calibration", packet["foreground_text"])
        self.assertIn("human-action layer", packet["foreground_text"])
        self.assertIn("derived momentum rising", packet["foreground_text"])
        self.assertNotIn("active layer", packet["foreground_text"])
        self.assertEqual(packet["calibration"]["status"], "initial_calibration")
        self.assertFalse(packet["calibration"]["source_truth"])
