...
from aippocampus_runtime.recall.ambient.card_ranking import rank_cards_for_brief
from aippocampus_runtime.recall.ambient.ref_counts import reopenable_ref_count
...
def with_card_provenance(
    ...
    *,
    ...
    clean["reopenable_ref_count"] = reopenable_ref_count(clean)
    if cache_status:
        clean["cache_status"] = {
            key: value
            for key, value in {
...
def _working_memory_card(item: dict[str, Any]) -> dict[str, Any]:
    ...
    card: dict[str, Any] = {
        ...
        "public_authority_tier": item.get("public_authority_tier"),
        "delivery_source_posture": item.get("delivery_source_posture"),
        "delivery_next_action": item.get("delivery_next_action"),
        "expand_if": (
...
def working_memory_card_match_key(item: dict[str, Any]) -> tuple[str, str]:
    """Return the stable public key used to attach private reopen refs.

    Prompt-hook working-memory rows deliberately strip source refs before they
    become foreground payload. Ambient source promotion still needs an internal
    way to match the stripped card back to its pre-strip source refs, otherwise
    a route can look reopenable while follow-through is impossible.
    """

    return (str(item.get("route") or ""), _theme_from_item(item, "soft working memory"))
...
def ambient_recall_from_decision(
    ...
    *,
    ...
    cards, brief_precision = rank_cards_for_brief(cards, prompt=prompt)
