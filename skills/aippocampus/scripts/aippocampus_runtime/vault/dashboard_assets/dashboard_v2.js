...
(() => {
  ...
  const generatedBodyGlobalAttrs = new Set(["class", "data-callout", "data-heading", "data-note", "id"]);
  ...
  function sanitizeGeneratedBodyHtml(bodyHtml) {
    const template = document.createElement("template");
    template.innerHTML = String(bodyHtml || "");
    // Pane bodies are generated by dashboard.py as a tiny HTML subset, then
    // read back from JSON text. Keep this as a rendering-boundary allowlist,
    // not a semantic classifier or a convenience hook for arbitrary HTML.
    Array.from(template.content.querySelectorAll("*")).forEach((element) => {
      const tag = element.tagName.toLowerCase();
      if (!generatedBodyAllowedTags.has(tag)) {
        element.replaceWith(document.createTextNode(element.textContent || ""));
        return;
      }
      Array.from(element.attributes).forEach((attr) => {
        if (!isGeneratedBodyAttrAllowed(tag, attr.name, attr.value)) {
  ...
  function publishBody(bodyHtml) {
    const safeBody = sanitizeGeneratedBodyHtml(bodyHtml);
