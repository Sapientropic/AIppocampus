...
from aippocampus_runtime.hooks.claude_code_handler import (
    ...
    handler_entry,
    is_aippocampus_handler_command,
    proposed_hooks,
)
from aippocampus_runtime.hooks.claude_code_handler import (
    handler_command as _handler_command,
)
    ...
    for event, groups in hooks.items():
        ...
        for group in groups:
            ...
            for handler in handlers:
                ...
                if is_aippocampus_handler_command(command):
...
def _is_aippocampus_handler(handler: Any) -> bool:
    command = str(handler.get("command") or "") if isinstance(handler, dict) else ""
    return is_aippocampus_handler_command(command)
