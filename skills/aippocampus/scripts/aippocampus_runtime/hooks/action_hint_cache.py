...
from aippocampus_runtime.hooks.action_hint_recovery import empty_cache_recovery
from aippocampus_runtime.hooks.recent_recall_routes import load_default_recent_recall_routes
...
def refresh_action_hint_cache(
    *,
    ...
    include_default_recent_recall: bool = True,
) -> dict[str, Any]:
    ...
    recent_recall_intake: dict[str, Any] = {
        "status": "not_requested",
        "source": "none",
        "route_count": 0,
    }
    effective_recent_recall_routes: Iterable[Mapping[str, Any]] | None = recent_recall_routes
    if (
        effective_recent_recall_routes is None
        and include_default_recent_recall
        and default_cache_path_used
        and root == Path.cwd().resolve()
    ):
        loaded_recent, recent_report = load_default_recent_recall_routes(now_unix=now_value)
        effective_recent_recall_routes = loaded_recent
        recent_recall_intake.update(recent_report)
    elif effective_recent_recall_routes is not None:
        materialized_recent = [
            dict(row) for row in effective_recent_recall_routes if isinstance(row, Mapping)
        ]
        effective_recent_recall_routes = materialized_recent
        recent_recall_intake.update(
            {
                "status": "found" if materialized_recent else "not_found",
                "source": "explicit_recent_recall_routes",
                "route_count": len(materialized_recent),
            }
        )
    report = build_action_hint_cache_report(
        ...
        recent_recall_routes=effective_recent_recall_routes,
        now_unix=now_value,
    )
    result: dict[str, Any] = {
        "ok": True,
        ...
        "recent_recall_intake": {
            **recent_recall_intake,
            "prepared_record_count": report["provider_counts"].get("recent_recall_route", 0),
        },
        "privacy_boundary": report["privacy_boundary"],
    }
    result["action_hints_ready"] = int(report.get("record_count") or 0) > 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--recent-recall-routes-json", type=Path)
    parser.add_argument("--no-default-learning", action="store_true")
    parser.add_argument("--no-default-recent-recall", action="store_true")
    ...
    result = refresh_action_hint_cache(
        ...
        recent_recall_routes=_load_provider_rows(args.recent_recall_routes_json),
        ...
        include_default_recent_recall=not args.no_default_recent_recall,
    )
