...
def _semantic_guidance_summary() -> dict[str, Any]:
    """Summarize deterministic semantic guidance without importing the CLI frontdoor.

    `learning_loop.cli` imports this module, so refresh-cache must not call back
    into the CLI helper. Keep this local lookup narrow: it only decides whether
...
def _empty_cache_recovery(
    *,
    ...
    semantic_summary = _semantic_guidance_summary()
    semantic_count = int(semantic_summary.get("semantic_action_time_guidance_count") or 0)
    if semantic_count:
        reason = "semantic_guidance_present_but_not_materialized"
