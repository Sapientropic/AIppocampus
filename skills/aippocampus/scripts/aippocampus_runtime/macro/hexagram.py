...
def _gray_code_lines(index: int) -> Lines6:
    gray = index ^ (index >> 1)
    return tuple((gray >> position) & 1 for position in range(6))  # type: ignore[return-value]


def _line_distance(left: Lines6, right: Lines6) -> int:
    return sum(1 for a, b in zip(left, right, strict=True) if a != b)


def _build_gray_walk_sequence() -> tuple[Hexagram, ...]:
    return tuple(hexagram_from_lines(_gray_code_lines(index)) for index in range(64))
...
def gray_walk_index(value: HexagramRef) -> int:
    item = resolve_hexagram(value)
    return _GRAY_WALK_INDEX_BY_LINES[item.lines]


def hexagram_by_gray_walk_index(index: int) -> Hexagram:
...
def gray_walk_prev(value: HexagramRef, *, wrap: bool = False) -> Hexagram | None:
...
def gray_walk_next(value: HexagramRef, *, wrap: bool = False) -> Hexagram | None:
    index = gray_walk_index(value)
...
def king_wen_pair_relation(value: HexagramRef) -> dict[str, Any]:
    item = resolve_hexagram(value)
    ...
    mate = hexagram_by_number(mate_number)
