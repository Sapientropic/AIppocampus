...
def print_help(*, file: TextIO | None = None) -> None:
    ...
    print("  pulse -> one-line readiness; status -> summary/current posture; health -> diagnostics/operator view", file=target)
    ...
    print("  observatory         Route-readiness dashboard, read-only", file=target)
    print("  episode-arcs        Sequence/arc continuity readout", file=target)
    print("  navigate            Boundary card for navigation sidecars", file=target)
    print("  telepathy           Local handoff coordination cards", file=target)
    ...
    print("  warm status         Inspect warm ambient queue without model calls\n  dream status        Background inference candidates, status only", file=target)
