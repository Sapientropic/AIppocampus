...
def main() -> int:
    ...
    graphify_corpus = resolve_artifact_path(
        args.graphify_corpus, cwd, default_thread_graphify_corpus_dir(cwd, rollout)
    )
    segments_dir = resolve_artifact_path(
        args.segments_dir, cwd, default_thread_segments_dir(cwd, rollout)
    )
    checkpoint_state = resolve_artifact_path(
        args.checkpoint_state, cwd, default_thread_checkpoint_state_path(cwd, rollout)
    )
    ...
    registry_path = (
        ...
        if args.registry
        else registry.registry_paths(Path(args.registry_dir).resolve() if args.registry_dir else None)[0]
    )
