"""Owner-specific payload compaction for dream working-memory rows."""
...
DREAM_WORKING_MEMORY_COMPACTION_MANIFEST_KIND = (
    "aippocampus_dream_working_memory_payload_compaction_manifest"
)
WORKING_MEMORY_KIND = "aippocampus_working_memory"
WORKING_MEMORY_SURFACE_KIND = "working_memory"
...
def _row_identity(row: Mapping[str, Any]) -> Any:
    return row.get("candidate_key") or row.get("surface_id") or row.get("id")
...
def _source_finding_count(row: Mapping[str, Any]) -> int:
    return len(row.get("source_finding_ids") or []) if isinstance(row.get("source_finding_ids"), list) else 0


def _dead_letter_update_safety_errors(update: Mapping[str, Any]) -> list[str]:
    errors: list[str] = []
    if update.get("surface_kind") != WORKING_MEMORY_SURFACE_KIND:
        errors.append("not_working_memory")
...
def _invalid_manifest() -> dict[str, Any]:
    return {
        ...
        "kind": DREAM_WORKING_MEMORY_COMPACTION_MANIFEST_KIND,
        "ok": False,
        ...
        "skipped": [
            {
                ...
                "surface_kind": WORKING_MEMORY_SURFACE_KIND,
            }
        ],
        "metrics": {
...
def _safe_update_map(
    ...
    for item in manifest.get("skipped") or []:
        if isinstance(item, Mapping) and item.get("surface_kind") == WORKING_MEMORY_SURFACE_KIND:
            skipped.append(
                {
    ...
    for update in manifest.get("updates") or []:
        if not isinstance(update, Mapping) or update.get("surface_kind") != WORKING_MEMORY_SURFACE_KIND:
            continue
...
def _compacted_working_memory_row(
    row: Mapping[str, Any],
    ...
    *,
    ...
    return {
        ...
        "candidate_type": compact_text(str(row.get("candidate_type") or ""), 80),
        "route": "park",
        "ask_policy": "do_not_use_compacted_payload",
        "visibility_tier": "dead_lettered_payload_compacted",
        "truth_boundary": "compacted_working_memory_tombstone_not_fact",
        "source_ref_count": source_ref_count,
        "source_finding_count": _source_finding_count(row),
...
def _compact_rows(
    ...
    *,
    ...
    for row in rows:
        ...
        if (
            update is None
            or row.get("kind") != WORKING_MEMORY_KIND
            or row.get("payload_compacted")
        ):
            next_rows.append(dict(row))
            continue
        tombstone = _compacted_working_memory_row(row, update, compacted_at=compacted_at)
    ...
    *,
    ...
    return {
        ...
        "metrics": {
            ...
            "working_memory_row_count": total_rows,
...
def compact_dream_working_memory_payloads_from_dead_letter_manifest(
    rows: Iterable[Mapping[str, Any]],
    dead_letter_manifest: Mapping[str, Any],
    *,
    compacted_at: str | None = None,
) -> tuple[list[dict[str, Any]], dict[str, Any]]:
    """Return transformed working-memory rows plus a public-safe compaction report.

    The generic activation audit emits hash-only lifecycle manifests. This
    owner-specific transform can match those hashes to working-memory
    `candidate_key` values, but the public report never serializes raw
    working-memory title, summary, trigger terms, source refs, or local paths.
    A caller that owns `working_memory.jsonl` may write the returned rows during
    maintenance; foreground hooks must not run this transform.
    """
        ...
        skipped.append(
            {
                ...
                "surface_kind": WORKING_MEMORY_SURFACE_KIND,
                "skip_reason": "working_memory_row_not_found",
            }
        )
