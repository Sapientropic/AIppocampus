...
orientation-only use guidance, and a reopen/deepen boundary. Broad action
grammar boilerplate, cache hit/miss counts, sidecar diagnostics, route ids,
source-generation digests, alias-source breakdowns, and candidate-ref counts
belong in public debug payloads, Observatory, or explicit deepen/audit paths,
not in default `additionalContext`.
